...
[예약 신청 완료 — 상태: pending_deposit]
  화면에 안내: "예약 신청이 접수됐어요. 아래 계좌로 입금해주시면 사장님
  확인 후 확정 문자가 발송됩니다." + 파트너의 계좌 정보(은행/계좌번호/
  예금주) + 입금 기한(예: 24시간 이내) 노출
        │
        ▼
[대기] 사장님이 입금 확인 → 카카오 알림톡으로 "예약 확정" 알림 수신
       (기한 내 미입금 시 자동 cancelled 전환, 3.3절)
```
...
- **벤더 선정 필요(미해결 — 4장 참고)**: 카카오 비즈메시지를 직접 붙이려면
  카카오톡 채널/발신프로필 심사가 걸려 통상 중계사(예: 솔라피, 알리고,
  NHN Cloud, 비즈엠 등)를 거친다. 이 프로젝트는 아직 이 중 어느 것도
  연동한 적이 없다(실측 확인, 0건) — 이번 문서에서 특정 벤더를 확정하지
  않는다(추측 금지, 사용자 확인 필요).
- **아키텍처는 결정 가능**: 벤더가 무엇이든 이 프로젝트의 기존 배치 관례를
  그대로 따르면 된다 — `scripts/ingest/event-reservation-reminder-push-batch.mjs`
  (10분 주기 cron, "지금 시점 기준 알림 대상"을 조회해 발송하고
  `*_sent_at` 컬럼으로 중복 발송 방지)와 동일한 패턴.
  - 신규 `scripts/partner/booking-notification-batch.mjs`(제안):
    ...
    2. **입금 기한 만료**: `status='pending_deposit'`이고
       `deposit_due_at < now()`인 예약을 `cancelled`로 전환(회차 잔여석
       자동 복구) + 고객에게 "입금 기한이 지나 예약이 취소됐어요" 안내.
- **우천 등 사유 일괄 취소/공지**(v1 스펙 7장, 미구현): 파트너 캘린더
  ...
  자동 전환, 벤더 기능에 위임) 일괄 발송. 이 버튼은 알림톡 벤더 연동이
  선행돼야 실제 발송까지 완성된다.
...
4. `bookings`에 `session_id`, `age_breakdown jsonb`, `deposit_due_at`,
   `deposit_amount`, `reminder_sent_at` 추가 + `status` CHECK에
   `pending_deposit` 추가 + `source` CHECK에 `nadripik_link` 추가
...
```
> 기존 온보딩 완료 파트너는 이 4개 컬럼이 전부 null로 남는다 — 온보딩 폼
> 재작성 시 "미입력 시 안내 배너로 재입력 유도" 같은 마이그레이션 UX를
> 별도로 결정해야 한다(이번 문서 범위 밖, 실제 구현 시점에 논의).
...
- 공개 예약 랜딩페이지(`/r/[slug]`) — 나드리픽 링크/앱 내부 버튼 공용
...
## 6. 미해결 의사결정 (사용자 확인 필요)

1. **알림톡/SMS 벤더** — 솔라피/알리고/NHN Cloud/비즈엠 중 예산·물량에 맞는
   곳 선정 필요. 벤더별 API 스펙이 달라, 벤더가 정해져야 3.3절 배치를
   실제로 구현할 수 있다.
2. **입금 기한 정책** — "24시간 이내" 등 구체적인 시간을 확정해야
   `deposit_due_at` 계산 로직(예약 시각 + N시간)을 정할 수 있다.
3. **`slug` 생성 규칙** — 농장명을 그대로 슬러그화(로마자 변환? 영문 입력?)
   할지, 사장님이 직접 원하는 문자열을 고를 수 있게 할지.
4. **기존 온보딩 완료 파트너의 계좌 정보 소급 입력** — 배너 유도 방식으로
   충분한지, 필수 재입력 게이트를 걸지.
