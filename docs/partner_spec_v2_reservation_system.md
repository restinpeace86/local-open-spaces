...
[예약 신청 완료 — 상태: pending_deposit]
  화면에 안내: "예약 신청이 접수됐어요. 아래 계좌로 10분 이내 입금해주시면
  사장님 확인 후 확정 알림이 발송됩니다." + 파트너의 계좌 정보(은행/
  계좌번호/예금주) + 입금 기한(신청 시각 + 10분) 카운트다운 노출
        │
        ▼
[대기] 사장님이 입금 확인 → 카카오 알림톡으로 "예약 확정" 알림 수신
       (10분 내 미입금 시 자동 cancelled 전환 + 안내, 3.3절)
```
> **결정(2026-09-25 수정)**: 입금 기한은 30분 → **10분**으로 단축. 자체 PG
> 없이도 예약과 "거의 동시에" 결제가 체감되도록, 대기 시간을 더 짧게 잡아
> 이탈을 최소화한다.
...
> **결정(2026-09-25 수정)**: 벤더는 Solapi로 확정하되, **실제 연동(계정
> 개설·과금 시작)은 지금 하지 않는다** — "비용이 들어가니 당장은 말고"
> (사용자 확인). 이 절의 비교/선정 내용은 **나중에 실제로 붙일 때 그대로
> 쓸 결정**으로 남겨두고, 이번 라운드 MVP에서는 알림톡이 필요한 기능들
> (새 예약 접수 알림, 예약 확정 알림, 노쇼 리마인드)을 전부 "다음 단계"로
> 미룬다(5장 참고). 지금 만드는 `pending_deposit`/입금 기한 로직 자체는
> 알림 발송과 무관하게 먼저 완성해둘 수 있다 — 나중에 벤더를 붙이는 순간
> 알림 트리거만 얹으면 된다(제5장 제4조, 확장 가능한 구조는 미리 잡되
> 지금 만들지 않는 것과는 별개).
...
| 입금 기한 만료 안내 | 고객 | 배치(신청 + 10분 경과) | 신규 |
...
- **배치**: `scripts/ingest/event-reservation-reminder-push-batch.mjs`(10분
  ...
  - 신규 `scripts/partner/booking-notification-batch.mjs`(제안):
    ...
    2. **입금 기한 만료**: `status='pending_deposit'`이고
       `deposit_due_at < now()`(= 신청 시각 + 10분)인 예약을 `cancelled`로
       전환(회차 잔여석 자동 복구) + 고객에게 "입금 기한이 지나 예약이
       취소됐어요" 안내. 10분짜리 기한이라 정밀도가 더 중요해졌으므로 이
       배치는 **1~2분 주기**로 돌리는 걸 권장한다(기존 10분 주기 배치보다
       훨씬 촘촘해야 함).
...
> **2026-09-25 수정**: Solapi 실제 연동은 비용이 발생해 당장 진행하지
> 않기로 했다("비용이 들어가니 당장은 말고") — 알림톡이 필요한 항목은
> 전부 "다음 단계"로 내렸다. `pending_deposit`/입금 기한 로직 자체는
> 알림 발송 유무와 무관하게 지금 완성 가능해 MVP에 남긴다.

**이번 라운드(MVP) 포함**:
- 계좌 정보 온보딩 필드 3종(신규 파트너부터 필수)
- `pending_deposit` 상태값 + 사장님 "입금 확인" 버튼(수동 확인, 알림 없이도
  화면에서 상태 전환은 가능)
- **입금 기한(10분) 만료 자동 취소 배치**(고객 알림은 다음 단계 — 우선
  회차 잔여석을 복구하는 상태 전환 로직만 완성)
- 회차/정원(연령 옵션 없이도 동작하는 기본형)
- 공개 예약 랜딩페이지(`/r/[slug]`, 농장명 기반 자동 슬러그) — 나드리픽
  링크/앱 내부 버튼 공용

**다음 단계(Solapi 실제 연동 시점에 함께 진행)**:
- 새 예약 신청 접수 알림(파트너 대상, 동기 트리거)
- 예약 확정 알림(고객 대상, 입금 확인 버튼 클릭 시 동기 트리거)
- 입금 기한 만료 안내(고객 대상)
- 노쇼 자동 리마인드 배치(방문 전날) — v1 스펙에 이미 명시
- 우천 일괄 취소 + 공지 발송 버튼 — 파트너 캘린더 UI 신규 개발도 함께 필요
...
2026-09-24 확인, 2026-09-25 일부 수정:
1. **알림톡/SMS 벤더** → Solapi로 **선정은 확정**(3.3절 비교표), 단
   **실제 연동은 비용 발생 시점까지 보류**(다음 단계).
2. **입금 기한 정책** → 30분 → **10분**으로 재확정, 초과 시 자동 취소 +
   회차 잔여석 복구.
