...
export function parsePriceFromText(text) {
  ...
  // [실측 버그 수정](2026-09-16, "트레저헌터 in 진안" 사용자 보고 — src/lib/admin/
  // parse-price-from-text.ts와 동일 수정) — 아래 3순위(라벨+"무료")보다 이 2순위를
  // 먼저 확인하도록 순서를 바꿨다. 긴 블로그 본문은 메인 참가 패스는 유료, 부가
  // 이벤트는 별도로 무료인 것처럼 여러 소주제를 함께 다루는 경우가 많아, 글 뒤쪽의
  // 무관한 "참가비: 무료" 문구가 실제 메인 요금(얼리버드 2,000원 등)보다 먼저
  // 매칭돼 버리는 문제가 있었다 — 텍스트 어딘가에 실제 숫자 금액이 있다면 그쪽이
  // 항상 더 신뢰할 수 있는 증거이므로 "무료" 추정보다 우선한다.
  ...
  // 붙는 경우(2026-09-16 사용자 지적: "숫자 가격이 아니라서.. 가격 없는걸로 한거야?
  // 무료는 숫자가 아니어도 유료/무료 판별이기 때문에.. 데이터 없음이 아니고
  // 무료로 나와야 하는거 아니야?"). "무료 (일부 재료비 별도)"처럼 라벨과 무관하게
  // 등장하는 "무료"는 입장료 자체가 무료라는 뜻인지 알 수 없어 여전히 잡지 않는다 —
  // 라벨 뒤 짧은 범위(10자 이내)에서만 인정하고, 위 1/2순위가 이미 텍스트 전체에서
  // 숫자 금액을 못 찾았을 때만 이 추정으로 넘어온다.
