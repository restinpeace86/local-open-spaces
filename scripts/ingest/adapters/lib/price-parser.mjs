...
export function parsePriceFromText(text) {
  ...
  // 무료로 나와야 하는거 아니야?" — src/lib/admin/parse-price-from-text.ts와
  // 동일 수정, 두 런타임 로직을 동일하게 유지한다). "무료 (일부 재료비 별도)"처럼
  // 라벨과 무관하게 등장하는 "무료"는 입장료 자체가 무료라는 뜻인지 알 수 없어
  // 여전히 잡지 않는다 — 라벨 뒤 짧은 범위(10자 이내)에서만 인정해 "관람료는 성인
  // 10,000원, 미취학은 무료"처럼 이미 실제 금액이 낀 문장까지 잘못 묶지 않는다.
