...
export const CATEGORY_MAJ_OF = {
  ...
  산림여가: '자연 / 캠핑',
  공원탐방: '자연 / 캠핑',
  // [이벤트픽 대분류 개편](2026-09-05 사용자 지시): "공공 키즈카페" → "키즈놀이터"로
  // 개명하고 open_spaces 전용 "키즈카페"를 세 번째 중분류로 추가한다(src/lib/spaces/
  // category-maj-meta.ts와 "반드시 동일하게 유지" 지침 — 위 체험휴양마을 등과 동일한
  // 사례. 이 값도 이 어댑터 파이프라인이 직접 배정하지 않는 open_spaces 전용 값이다).
  공공키즈카페: '키즈놀이터',
  어린이실내놀이터: '키즈놀이터',
  키즈카페: '키즈놀이터',
  // [여기저기/일반 서울형키즈카페 구분](2026-09-20 사용자 지시): src/lib/spaces/
  // category-maj-meta.ts와 반드시 동일하게 유지(위 상단 지침).
  서울형키즈카페: '키즈놀이터',
  농장체험: '체험 / 농장',
  도시농업: '체험 / 농장',
  '자연/과학': '체험 / 농장',
  ...
  '교양/어학': '배움 / 클래스',
  교육시설: '배움 / 클래스',
  // [이벤트픽 대분류 6종으로 축소](2026-09-04 사용자 지시): src/lib/spaces/category-maj-meta.ts에서
  // "스포츠 대여" 대분류 자체를 제거했다("반드시 동일하게 유지"하라는 상단 지침에 따라
  // 이 파일도 맞춘다) — 이 15개 중분류는 이제 어떤 대분류에도 속하지 않는다(null). 아래
  // CATEGORY_MAJ_OF[key] ?? null 소비 지점 덕분에 null로 두면 카테고리 판정 로직이 그대로
  // "미분류"로 처리한다(에러 없음). '기타'/'청년정보' 등 기존 제외 항목과 동일한 처리 방식.
  테니스장: null,
  풋살장: null,
  축구장: null,
  체육관: null,
