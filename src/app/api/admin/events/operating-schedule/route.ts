...
// 관례(단일 id PATCH, table 구분 없음 — 이 두 컬럼은 events 전용).
...
function normalizeWeekdays(value: unknown): string[] | null {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { id, operating_weekdays: operatingWeekdays, excluded_weekdays: excludedWeekdays } = body as {
      id?: unknown;
    ...
    const { data, error } = await admin
      .from('events')
      .update({
        operating_weekdays: normalizeWeekdays(operatingWeekdays),
        excluded_weekdays: normalizeWeekdays(excludedWeekdays),
      })
      .eq('id', id)
      .select('id, operating_weekdays, excluded_weekdays')
      .single();
