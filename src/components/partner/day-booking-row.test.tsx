...
import { DayBookingRow } from './day-booking-row';

// [나드리픽 파트너 PMS — 주간 뷰](2026-09-20 사용자 지시): 예약 유무에 따른 표시,
// 취소 건 시각 구분, 오늘 뱃지, 일간 뷰로의 링크 연결을 검증한다.
// [주간 뷰를 월간으로 흡수](2026-09-23 사용자 지시): weekly-day-row.test.tsx를
// 이름만 일반화해 그대로 옮겼다 — 검증 내용은 동일(요일 개념과 무관한 순수
// 표시 컴포넌트).
describe('DayBookingRow', () => {
  it('예약이 없으면 "예약 없음"을 보여준다', () => {
    render(<DayBookingRow date="2026-09-22" bookings={[]} isToday={false} />);
  ...
  it('오늘 날짜면 "오늘" 뱃지를 보여준다', () => {
    render(<DayBookingRow date="2026-09-22" bookings={[]} isToday />);
