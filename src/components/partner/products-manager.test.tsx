...
const PRODUCT = { id: 'p1', name: '캠핑사이트 A형', price: 50000, pricing_unit: 'flat' as const };
...
describe('ProductsManager', () => {
  ...
  it('상품 목록을 이름/가격/가격 기준과 함께 보여준다', () => {
    ...
    expect(screen.getByText('50,000원 · 팀당(인원수 무관)')).toBeInTheDocument();
  });
  ...
  it('상품 추가 버튼을 누르면 폼이 열리고, 제출하면 createPartnerProduct를 호출한다', async () => {
    ...
    await waitFor(() =>
      expect(createMock).toHaveBeenCalledWith({ name: '입장권', price: 15000, pricing_unit: 'per_person' })
    );
  ...
  it('수정 버튼을 누르면 기존 값이 채워진 폼이 열리고, 제출하면 updatePartnerProduct를 호출한다', async () => {
    ...
    await waitFor(() =>
      expect(updateMock).toHaveBeenCalledWith('p1', { name: '캠핑사이트 B형', price: 50000, pricing_unit: 'flat' })
    );
  });
