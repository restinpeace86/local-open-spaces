    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      <div className="shrink-0 p-4 border-b border-gray-100 flex flex-col gap-3">
        ...
        {isOpsToolsExpanded && (
          <>
            <TodayBatchSummary />
            {/* [파이프라인 로그 DB화](2026-09-15 사용자 지시, todo.md [개선사항 3]):
                "상단에 산재해 있던 수동 재수집 기능을 이 탭으로 통합" — 개별 소스 수동
                재수집 UI(구 IngestRerunPanel)를 전용 관리자 탭(/admin/pipeline)으로
                이전하고, 여기서는 진입 링크만 남긴다. */}
            <a
              href="/admin/pipeline"
              className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-blue-600 hover:underline inline-flex items-center gap-1 w-fit"
            >
              🔁 파이프라인 관리(현황판 · 개별 소스 수동 재수집) →
            </a>
          </>
        )}
