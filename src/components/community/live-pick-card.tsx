...
export function LivePickCard({ post }: { post: DashboardPost }) {
  return (
    <Link
      href="/mom-pick/live"
      className="flex items-start gap-3 rounded-xl border border-red-100 bg-gradient-to-r from-red-50 to-white p-3 shadow-sm transition-shadow hover:shadow-md"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-xs text-gray-500">
          {post.author.nickname ?? '이름 없는 맘'} · <span className="font-medium text-emerald-600">{formatTimeAgo(post.created_at)}</span>
        </p>
      </div>
    </Link>
  );
