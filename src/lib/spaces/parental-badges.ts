...
function getEventBadges(item: NearbyItem): ParentalBadge[] {
  ...
  // [이벤트 카드 텍스트 영역 뱃지 정리](2026-09-04 사용자 지시): "키즈/어린이" 뱃지가
  // 텍스트 영역 자리를 차지하고 거슬린다는 지적에 따라 이벤트 카드에서는 완전히
  // 제거한다(예전엔 여기서 'kids' 뱃지를 만들었다 — Quick Filter 스크리닝 기준과
  // 뱃지를 일치시키려던 목적이었으나, 이벤트픽은 애초에 가족/아동 대상 콘텐츠만
  // 다루므로(EVENT_PICK_TARGET_AUDIENCES) 카드마다 반복 표시할 실익이 적었다). 이
  // 카드가 참조하던 EventCard의 hideBadgeKeys 특례(ReservationOpenSlider용)도 이제
  // 존재 이유가 없어져 함께 제거했다 — getSpaceBadges(스팟픽 화면)의 'kids' 뱃지는
  // 이번 지시 대상이 아니므로 그대로 둔다.
