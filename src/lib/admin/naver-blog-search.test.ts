...
describe('buildSmartBlogQuery', () => {
  ...
  // [지역명 중복 방지](2026-09-07 사용자 지시): "임성근국가공인진갈비 김포점..
  // 여기에 김포를 뒤에 붙여서 임성근국가공인진갈비 김포점 김포로 호출중인거
  // 같더라.. 임성근국가공인진갈비 김포 이렇게 조회 호출하도록 해"
  it('상호명이 이미 "[지역명]점"으로 끝나면 지역명을 또 붙이지 않고 "점"만 뗀다', () => {
    expect(buildSmartBlogQuery('임성근국가공인진갈비 김포점', '경기도 김포시')).toBe('임성근국가공인진갈비 김포');
  });

  it('상호명이 지역명을 포함하지만 "점"으로 끝나지 않으면 그대로 지역명을 붙인다', () => {
