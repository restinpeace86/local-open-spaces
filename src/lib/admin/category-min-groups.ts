...
export const OPEN_SPACES_GROUPS_STATIC: CategoryMinGroup[] = [
  ...
  // [설치장소코드 표준 중분류 전면 정비 후속](2026-09-09 사용자 지시): "키즈/놀이시설쪽에
  // 육아종합지원센터와 유아교육진흥원 중분류는 왜 기타 대분류로 안옮겼어?" — 개선사항3에서
  // 함께 정리한 나머지 14개 신규 중분류(목욕장업소/도로휴게시설/식품접객업소/아동복지시설/
  // 어린이집/유치원/대규모점포/의료기관/주택단지/학교/학원/주상복합/종교시설/하천)는 전부
  // 정적 그룹 정의에 없어 buildCategoryMinGroups()가 자동으로 '기타'에 편입하지만(83행
  // unassigned 로직), 육아종합지원센터/유아교육진흥원은 예전부터 '키즈/놀이시설'에
  // 명시적으로 들어있어 자동 편입 대상이 아니었다 — 실제로는 "육아 정책/행정 지원기관"이라
  // "실제 놀이시설"인 다른 키즈/놀이시설 항목들과 성격이 달라, 나머지 14개와 같은 기준으로
  // '기타'로 옮긴다.
  { major: '기타', minors: ['기타', '민원 등 기타', '육아종합지원센터', '유아교육진흥원'] },
];
...
const EVENTS_GROUPS_STATIC: CategoryMinGroup[] = [
  ...
  {
    major: '문화/축제',
    minors: ['전시/관람', '미술제작', '공예/취미', '역사', '문화행사', '지역축제/페스티벌'],
  },
  {
    major: '자연/체험',
  ...
  {
    // [원천 대/중분류 재분류 반영](2026-09-12 사용자 지시): "문화/축제 >> 전시실
    // 항목 있는데 이거 원천 대/중분류가 공간시설/전시실인데? 이것도 공공청사/행정
    // 쪽으로 옮겨줘.. 여기의 표준 중분류 조건은.. 전혀 반영 안됐어.. 여기도
    // 반영해줘" — events.category_maj를 실제로 '공공청사/행정'으로 옮긴 것만으로는
    // 이 어드민 필터 그룹핑(이 파일, DB 컬럼과 무관한 정적 UI 전용 그룹)에 반영되지
    // 않는다는 걸 실측으로 확인했다(전시실이 여전히 위 '문화/축제'의 minors에
    // 나열돼 있었음). '전시실'을 여기로 옮겨 실제 반영한다.
    // [후속](2026-09-12 사용자 지시): "문화/축제 >> 공연장도 공간시설 대관이야..
    // 이것도 공공청사/행정의 공연장으로 옮겨줘.. DB는 물론 관리자 화면도" — 실측
    // 확인 결과 DB는 이미 공간시설 소스 45건이 '공공청사/행정'으로 반영돼 있었고
    // (문화행사 API 등에서 온 4건만 실제 공연 성격이라 그대로 둠), 이 그룹핑만
    // 안 맞았던 것과 동일한 패턴 — '공연장'도 여기로 옮긴다.
    major: '공공청사/행정',
    minors: [
      '강당', '강의실', '다목적실', '회의실', '주민공유공간', '녹화장소', '청년공간', '청년정보',
      '단체봉사', '보건소', '서북병원', '어린이병원', '장애인버스', '전시실', '공연장',
    ],
  },
