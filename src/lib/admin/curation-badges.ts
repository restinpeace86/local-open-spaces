...
// [이벤트픽 블로그 큐레이션 하이라이팅 — 아이 관심 요소](2026-09-11 사용자 지시):
// "아이 라던가 이런 키워드도.. 분수, 빛, 불꽃 등등 아이들이 좋아할만한 공연 단어라던가도
// 키워드로.. '아이들과 가기 좋은' '아이들이 너무 좋아하는' 과 유사한 문구에도.. 이야기..
// 공연도.. 동화라던가" — 관리자가 이벤트픽(가족/아동 대상) 적합 여부를 빠르게 훑어볼 수
// 있도록, 대상 지시어·감성 문구·볼거리 테마 단어를 함께 형광펜 처리한다. 문구(예:
// "아이들과 가기 좋은")도 하이라이트 키워드와 동일하게 공백 무시 매칭이 그대로
// 적용된다(highlightKeywordsOnly는 여러 단어 키워드도 문자 단위 공백 무시로 매칭).
export const KID_APPEAL_HINT_KEYWORDS: string[] = [
  // 대상 지시어
  '아이', '아이들', '아이랑', '아이와', '아이 동반', '자녀', '어린이',
  // 감성 문구
  '아이들과 가기 좋은', '아이랑 가기 좋은', '아이들이 좋아하는', '아이들이 너무 좋아하는',
  '가족 나들이', '가족과 함께', '아이와 함께',
  // 볼거리/테마
  '분수', '불꽃놀이', '야경', '동화', '이야기', '공연', '인형극', '마술쇼', '캐릭터',
];
