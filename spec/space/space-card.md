...
- **가성비(필수)** `is_free`**:
  - `is_free === true` ➔ `🎁 무료` 뱃지 노출
  - `is_free === false` ➔ `💰 유료` 뱃지 노출 — 유료인 경우 1만원 이하 여부는 상세 모달에서 안내
  - `is_free === null` (정보 미기재/알 수 없음) ➔ **요금 뱃지 숨김 (미노출)**
    > *주의: TourAPI 등 원천 데이터의 요금 필드 부재로 인한 null 값을 유료(false)로 단정해 오탐 표출하지 않는다.*
