...
> **상태: DRAFT — 세부 쟁점 확정 완료, Decision 기록 대기.** 이 문서는 사용자가
> 채팅으로 전달한 기획 원문을 구조화하고, 2026-09-02 사용자 답변으로 모든 쟁점
> (채택 정의/VAPID 발급 주체/등급 산정 주기/리뷰·체크리스트 입력 형태/파워맘 선발
> 방식/강등 정책)을 확정한 것이다(3절). 사용자가 `project/decision-log.md`에
> Decision으로 승인 기록을 남긴 뒤에 구현에 들어간다(제3장 제2조 Spec 우선 /
