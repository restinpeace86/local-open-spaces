...
6. **"본문 텍스트" 범위 조정(실측 기반)**: 네이버 블로그 검색 API는 전체 본문이
   ...
   있어 별도의 페이지 구조 확인 없이 안정적으로 크롤링할 수 없다(추측 금지). 이번
   구현은 검색 API가 실제로 제공하는 요약 스니펫을 정제해(네이버 자체 `<b>` 태그 제거)
   보여주고, 하이라이팅은 그 위에 적용한다 — "전체 본문 스크래핑"은 이번 범위에
   포함하지 않으며, 필요하면 별도 Decision/Spec으로 다룬다.
