...
## Decision 013
### 제목
이벤트/공간 데이터 스펙 확장 승인 (가격 정보 세분화 및 예약 접수 기간 분리)

### 결정 내용
1. 이벤트 및 공간의 비용 구조를 기존 단순 무료(`is_free`) 여부 외에 AI 기반 세부 금액 정보를 저장/노출할 수 있도록 `price_info` 필드를 표준화한다.
2. 행사의 기간(`start_date` ~ `end_date`)과 별개로 **사전 예약 접수 기간(`reservation_start_date` ~ `reservation_end_date`)** 데이터를 파이프라인 및 UI 스펙에 독립 필드로 명시하여 예약 마감 카운트다운 및 예약 상태 태그(`예약 필요`/`예약 필요없음`/`예약 기간 시작전`/`예약 마감`)를 정확히 제공한다.
