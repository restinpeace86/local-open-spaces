...
6. **[정정] "본문 텍스트" 범위(당초 추측 → 실측으로 재확인, 2026-09-05)**: 네이버
   ...
   3항 정책 그대로).
7. **[신규] 검색 결과 품질 문제 확인(실측, 코드로 해결 불가)**: "호박터숯불촌"
   등 실제 스팟명으로 직접 재현 테스트한 결과, NAVER API HUB의 관련도순
   (`sort=sim`) 블로그 검색이 스팸성 블로그(무관한 키워드 나열 게시물)를 상위에
   올리거나, 흔한 일반 검색어에서도 실제 네이버 블로그보다 티스토리 위주로
   결과를 편향되게 반환하는 사례를 여러 쿼리에서 재현 확인했다 — 이는 네이버
   API HUB(2026-06-25 신규 이관, 아직 초기 단계) 자체의 검색 품질 문제로 판단되며,
   우리 쪽 쿼리 문구를 바꿔도(따옴표 강제 매칭, 지역명 추가 등 시도) 결과가
   달라지지 않아 클라이언트 코드로 교정할 방법이 없다. 대신 관리자가 직접 찾은
   정확한 URL로 현재 탭을 바꿔치기할 수 있는 "다른 URL로 바꾸기" 기능을 추가해
   실용적인 탈출구를 제공한다.
