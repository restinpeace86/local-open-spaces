...
- [x] **[Task 7-3] 전국 수영장(공공+민간 인허가) 통합 수집 어댑터 구현**
  ...
  - **`is_kids_friendly` 미매핑 사유**: 두 API 모두 개별 레코드가 "키즈 전용"인지 구분하는 필드가 없음(수영장은 어린이놀이시설과 달리 소스 전체가 아동 전용이라고 단정할 근거가 없음 — 성인 강습/자유수영 등 일반 대상이 대부분) → 추측 금지 원칙에 따라 기본값(false) 유지.
  ...
  - **검증**: `swimming-pool-adapter.test.mjs` 14/14 통과, `npx tsc --noEmit`/`npm run test`(60/60)/`npm run build` 모두 통과, 실제 API 호출로 dry-run(1,537건) 후 실제 upsert 완료(1,537건, 중복 0건 확인).
