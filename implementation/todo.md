...
### ⏭️ `extractReservationsFromPage` 실제 셀렉터 구현 — 스킵 처리 (2026-09-21)
**① 상세 스킵 사유**: 사용자가 "`--debug` 실행으로 확보한 HTML 구조를 참고해
실제 셀렉터를 작성해 달라"고 요청했으나, 리포지토리와 이 세션 어디에도 실제
캡처 결과물이 없음을 확인했다(`scripts/partner/.debug/` 디렉터리 자체가 아직
생성된 적 없음, HTML/스크린샷이 공유된 적도 없음). 사용자에게 직접 확인한 결과
**"스태프 위임이 아직이라 보류"** — 위 재개 기록에 이미 명시된 선행 조건(스태프
권한 위임 완료 → `--debug` 캡처 → 결과 공유)이 아직 충족되지 않았다. 실제 페이지
구조 없이 CSS 셀렉터를 작성하면 추측이 되고(제3장 제5조 추측 금지), 틀린
셀렉터는 "0건 추출"로 조용히 실패해 문제를 숨기므로 진행하지 않았다.
**② 재개 전 선행 작업**: (1) 파트너 업체가 네이버 예약 시스템에 스태프 계정을
등록, (2) `NAVER_STAFF_SESSION_COOKIES_JSON` 설정 후 `node scripts/partner/
naver-reservation-sync-bot.mjs --debug` 실행, (3) `scripts/partner/.debug/`에
저장된 HTML/스크린샷(또는 그 내용)을 공유. 이후 재요청하면 즉시 이어서 진행
가능 — 프로세스 나머지 부분(세션 주입/업체 순회/upsert/로깅)은 이미 완성됨.
