...
- [x] **[버그 수정] 로딩 GIF 크기 확대 + 배경 색상 불일치 해결** (2026-09-03 완료)
  - "너무 작다"/"배경이 흰색이 아니라 주변과 다르다" 피드백. 원인 실측: 원본 배경이
    완전한 흰색(#fff)이 아니라 (252,252,255)였고, 32색 압축 과정에서 더 어긋났다.
  - 배경색 기준 색상거리 임계값으로 투명 처리 후 재인코딩(GIF는 반투명 불가 —
    완전 투명/불투명만), 해상도 320×180로 상향, 화면 표시를 `h-24 w-24
    object-contain`(96px 정사각, 16:9 원본이라 여백 낭비)→`w-40 h-auto`(160px,
    비율 그대로)로 변경.
  - 결과: 60,317→409,076 bytes(원본 3.15MB 대비 여전히 7.7배 경량).
  - **검증**: `npx tsc --noEmit`/`npm run test`(96파일 964건)/`npm run build` 통과,
    투명도 재확인 및 흰 배경 합성 미리보기 육안 확인, dev 서버 curl로 정상 서빙
    확인. 상세: `implementation/2026-09-03-loading-gif-size-and-background-fix.md`.
