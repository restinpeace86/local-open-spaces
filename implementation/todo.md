...
- [ ] **[Task 2] 전국도시공원정보표준데이터 수집 & `open_spaces` 표준화 어댑터 구현**
  ...
  - **스킵 유지 재확인 (2026-08-21, 13차 세션)**: `scripts/ingest/city-parks.mjs` 재확인 결과 여전히 동일 데이터셋(`api.data.go.kr/openapi/tn_pubr_public_cty_park_info_api`)을 `source_type='PARK_API'`로 수집 중 — Task 2 지시대로 `source_type='CITY_PARK_STANDARD'` 별도 어댑터를 신설하면 동일 전국 공원 데이터가 중복 적재됨. `project/decision-log.md`(Decision 001~008 전체) 재확인 결과 Task 2 재구현(중복 어댑터 신설)을 승인하는 Decision 여전히 없음 — 블로커 1(승인 Decision 부재) 상태 변화 없음. `.env.local`(41 라인) 재확인 결과 `VWORLD_API_KEY` 여전히 미존재 — 블로커 2도 상태 변화 없음. 두 블로커 모두 유지되어 스킵 유지. `npx tsc --noEmit` 통과(오류 없음), `npm run test`(4 files, 15 tests passed) 통과, `npm run build` 통과(정적 페이지 5개 생성 완료). 코드 변경 없음.
