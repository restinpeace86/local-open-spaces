...
- [x] **[10대 타겟 분류 체계 및 활성 데이터 실데이터 반영 및 성능 최적화]** — 완료(2026-08-27).
  ...
  - **[대표 승인 완료 사항 (잠정 규칙 5건)]**
    ...
    3. **`TEEN` 태그 룰**: `중고등` 및 문맥 제한된 `학생` 키워드 조합 적용.
    4. **`KIDS_SCHOOL` 태그 룰**: `키즈` 키워드 및 초등 관련 키워드 반영.
    5. **원천 필드 우선 탐색 (`raw_data`)**: 텍스트 파싱 전 `USE_TRGT`, `target_age_group`, `USETGTINFO`, `MINCLASSNM` 등 원천 필드를 0순위로 스캔하여 10대 태그 1:1 매핑 선행.
  - **[상세 작업 내용]**
    1. **본 데이터 마이그레이션 스크립트 실행 (`is_active = true` 대상)**: 
       - 승인된 10대 분류 룰과 원천 필드 우선 탐색 로직을 적용하여, 활성 상태의 `events` 데이터에 타겟 연령 및 중분류를 실제 DB에 `UPDATE` 반영.
    2. **어드민 UI 연동**:
       - `/admin/data-grid` 화면의 타겟 연령 및 분류 드롭다운/필터 UI에 10대 태그 체계(`ADULT`, `FACILITY` 포함) 완벽 연동.
    3. **이벤트픽 조회 쿼리 성능 최적화**:
...
- [x] **[1단계 중분류(Category Mid) raw_data 원천 필드 우선 탐색 및 Dry-run 시뮬레이션]** —
  ...
  - **[핵심 원칙 - 절대주의]**
    ...
    3. **NULL 잔여 및 매핑 정합성 분석**: 원천 필드 우선 매핑 적용 시와 기존 방식 비교를 통해 중분류 `NULL` 잔여 건수 및 분류 정확도를 측정하고 리포트를 산출한다.
  - **[상세 작업 내용]**
