## 🚨 자율 실행 및 작업 진행 지침 (Strict Execution Rules)

1. **GitHub `todo.md` 기반 작업 수행**: 본 문서에 명시된 Task 목록과 세부 작업 지시를 최우선 가이드라인으로 삼아 순차적으로 작업을 진행한다.
2. **충돌 발생 시 즉시 스킵 (Skip on Conflict)**:
   - 기존 Spec 문서 (`spec/`), Decision Log (`project/decision-log.md`), 또는 기존 모듈과 구조적/논리적 충돌이 발생하는 경우, 절대로 무리하게 코드를 수정하지 말고 즉시 **[스킵 (보류)]** 처리한다.
3. **스킵 처리 시 필수 기록 사항**:
   - 충돌로 인해 작업을 스킵할 경우, 해당 Task 하단에 **① 상세 스킵 사유**를 명확히 기록한다.
   - 해당 Task를 재개하기 위해 **② 선행되어야 할 작업**(예: 신규 Decision 기록 필요, Spec 문서 선행 수정 필요 등)을 구체적인 가이드로 명시한다.
4. **원격 문서 갱신 반영 및 동기화**:
   - 원격 저장소의 `project/decision-log.md` (Decision 010) 및 `spec/map/spatial-search.md` (2.2 레이어 분리) 변경 내역을 확인하고, 충돌이 해소된 상태에서 안전하게 다음 Task를 진행한다.
5. **결과 업데이트 및 정합성 유지**:
   - 작업 완료 시 관련 테스트/빌드를 검증하고 `todo.md` 내 체크박스(`[x]`) 및 진행 상태를 최신화한다.

---

### Task 목록
- [ ] **[Task 9-6-20] Decision 013 기준 6대 카테고리 개편, 인라인 피딩 & 경기도 기타 UI 제거 구현** 📱🎪
  - **세부 작업 지시**:
    1. **6대 카테고리 개편 및 인라인 피딩 구현**:
       - `docs/spec.md` 3.2절 및 Decision 013 기준에 따라 6대 카테고리(`👶 키즈`, `🎪 공연·축제`, `🎨 체험`, `📚 클래스`, `🏛️ 전시·박물관`, `🏟️ 공공시설 예약`)로 개편하고 탭 클릭 시 메인 화면 내부 인라인 피딩 처리.
       - 카테고리 선택 시 피드 최상단에 1줄 가이드 안내 문구 노출.
    2. **'경기도 기타' 및 '가성비 행복' 별도 UI 섹션 완전 제거**:
       - Decision 013 규정에 따라 메인 화면의 '경기도권 기타'(getProvinceWideEvents) 및 '가성비 행복' UI 섹션을 완전히 제거하고, UNKNOWN 데이터 포함 6대 카테고리 피드로 100% 통합 피딩 처리.
    3. **스팟픽 데이터 배제 및 실내/야외 뱃지 처리**:
       - 상시 야외/자연 스팟 데이터는 이벤트픽 피드에서 제외하고, 야외 여부는 카드 상단 `[☀️ 야외]` / `[🏛️ 실내]` 뱃지로 표기.
    4. **모바일 뒤로가기(PopState) 모달 제어**:
       - 상세 모달 오픈 시 `popstate` 연동을 통해 모바일 뒤로가기 실행 시 이전 페이지로 이동하지 않고 모달만 깔끔하게 닫히도록 처리.
