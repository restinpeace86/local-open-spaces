## 🚨 자율 실행 및 작업 진행 지침 (Strict Execution Rules)

1. **GitHub `todo.md` 기반 작업 수행**: 본 문서에 명시된 Task 목록과 세부 작업 지시를 최우선 가이드라인으로 삼아 순차적으로 작업을 진행한다.
2. **충돌 발생 시 즉시 스킵 (Skip on Conflict)**:
   - 기존 Spec 문서 (`spec/`), Decision Log (`project/decision-log.md`), 또는 기존 모듈과 구조적/논리적 충돌이 발생하는 경우, 절대로 무리하게 코드를 수정하지 말고 즉시 **[스킵 (보류)]** 처리한다.
3. **스킵 처리 시 필수 기록 사항**:
   - 충돌로 인해 작업을 스킵할 경우, 해당 Task 하단에 **① 상세 스킵 사유**를 명확히 기록한다.
   - 해당 Task를 재개하기 위해 **② 선행되어야 할 작업**(예: 신규 Decision 기록 필요, Spec 문서 선행 수정 필요 등)을 구체적인 가이드로 명시한다.
4. **원격 문서 갱신 반영 및 동기화**:
   - 원격 저장소의 `project/decision-log.md` (Decision 010) 및 `spec/map/spatial-search.md` (2.2 레이어 분리) 변경 내역을 확인하고, 충돌이 해소된 상태에서 안전하게 다음 Task를 진행한다.
5. **결과 업데이트 및 정합성 유지**:
   - 작업 완료 시 관련 테스트/빌드를 검증하고 `todo.md` 내 체크박스(`[x]`) 및 진행 상태를 최신화한다.

---

## 진행 상태 (Harness 자율 실행 — 2026-09-11 순차 처리)
- [x] 개선사항1 검토: 맘스픽 메인엔 이미 상단 장소 선택 바/검색창이 없음(확인 완료, 제거할 대상 없음).
      Gating은 Step 96·107에서 이미 구현 — guest 클릭 시 로그인 안내 팝업(LoginPromptModal, 이 앱에
      별도 "로그인 화면" 페이지가 없어 소셜 로그인 버튼이 포함된 안내 모달이 그 역할을 겸함),
      not_sprout_yet 클릭 시 첫 글 쓰기 유도(SaessakMomGuideModal), 정상 유저는 제약 없음 — 스펙과
      일치함을 재확인. 추가 코드 변경 없음.
- [x] 개선사항2 (Step 108): **긴급 DB 버그 수정** — `get_nearby_spaces_and_events` RPC가
      Step 82(2026-09-09) 이후 `coalesce(standard_name, name)` 타입 불일치(text vs varchar,
      PL/pgSQL RETURN QUERY는 암묵 캐스팅 안 함)로 SPACE를 포함한 모든 호출에서 100% 실패 중이었음
      (스팟픽 기본 지도·홈 피드·맘스픽 인기 스팟 등 전역 영향). `::character varying` 캐스팅 추가로
      수정, 프로덕션 DB 직접 호출로 검증 완료. (완료: 2026-09-11)
- [x] 개선사항3 (Step 109): "전체보기" 바텀시트 4곳(이벤트픽 대/중분류 선택=
      MajorCategoryGrid, "지금 이 순간 함께하기 좋은 알찬 픽"/"놓치면 후회하는 인기
      만점 예약 픽"/"오늘 전체보기"=EventBrowseSheet)의 이미지 포함 그리드(EventCard/
      FeedCard)를 이미지 없는 1열 리스트(`EventListRow` 신규, 4단 라인: 제목+거리 /
      뱃지 / 행사·운영기간 / 예약기간)로 통일. EventCard/FeedCard 자체는 다른 화면
      (슬라이더 등)에서 여전히 이미지가 필요해 그대로 유지, 새 전용 컴포넌트만 추가.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-eventpick-browse-sheets-image-free-list.md)
- [x] 개선사항4 (Step 110): MajorCategoryGrid — 대분류 클릭 시 첫 번째(왼쪽) 중분류
      자동 선택(onSelectMin 즉시 호출, 0건 중분류는 건너뜀)으로 결과 로딩이 바로
      시작되도록 개선. 두 바텀시트(MajorCategoryGrid/EventBrowseSheet) 모두 로딩
      스켈레톤을 신규 `EventListSkeleton`(1열 리스트 모양)으로 통일. 중복 fetch 점검
      결과 실제 중복 호출 지점 없음(코드 변경 없이 점검 결과만 기록).
      (완료: 2026-09-11, 상세: implementation/2026-09-11-eventpick-browse-sheets-autofocus-and-skeleton.md)
- [x] 개선사항5 (Step 111): "전체보기" 3곳(오늘/알찬 픽/예약 픽)에 스팟픽과 동일한
      도(道) 단위 1차 필터(province.ts 재사용) + 거리순 정렬(sortByDistanceIfKnown
      재사용) 적용. 신규 getTodayEventsPage(오늘 전체보기 전용, Hero 미리보기용
      getTodayEvents는 그대로 유지) + getCurrentlyOngoingEventsPage/
      getReservationOpenEventsPage를 fetchAllRowsChunked 기반으로 재작성(PostgREST
      max_rows=1000이 프로덕션에도 적용됨을 실측 확인 — 단일 range() 요청은 최대
      1,000건까지만 반환됨). EventBrowseSheet에 userLocation prop 추가로 실제 GPS
      좌표를 서버에 전달. MajorCategoryGrid(4번째, "검토" 대상)는 이미 distance 정렬
      중이라 이번엔 현황만 기록, 추가 변경 없음.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-eventpick-browse-sheets-distance-sort.md)
- [x] 개선사항6 (Step 112): 이벤트 상세카드를 8단 구조(이미지 슬라이드/뱃지(카테고리+
      진행상태)/제목+인터랙티브 거리/행사기간/예약기간/설명 토글/미니맵/하단 고정
      CTA)로 재설계. 스팟(SPACE) 분기는 완전히 그대로 유지, 기존 부가 정보(연령대상/
      예약 안내)는 삭제하지 않고 새 구조 안에 재배치.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-event-detail-card-8-section-restructure.md)
- [x] 개선사항7-2 + 8 (Step 113): 이벤트픽 블로그 큐레이션 관리자→DB→유저 파이프라인.
      `events.curated_blog_urls text[]`(최대 3개) 컬럼 신규, 관리자 Events 탭 상세
      팝업에 "블로그 큐레이션" 버튼(검색→체크박스 선택→저장), 유저 이벤트 상세
      화면(8단 구조 6단 아래·7단 위)에 "방문 후기/추천 블로그" 카드 조건부 노출(0건
      시 섹션 숨김). open_spaces의 기존 블로그 검색/본문 API(Decision 021)를
      그대로 재사용.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-event-blog-curation-admin-and-user.md)
- [x] 개선사항7-1 (Step 115): 가격 정보 파싱 고도화. 4개 이벤트 소스를 전부 실측
      조사한 결과 2곳(SEOUL_CULTURE_EVENTS의 USE_FEE, GG_CULTURE_EVENTS API1의
      PARTCPT_EXPN_INFO)은 이미 원본 API가 가격을 자유 텍스트로 제공해 크롤링
      불필요, 나머지(GG_CULTURE_EVENTS API2/SEOUL_YEYAK)는 설명 텍스트에서 라벨+
      금액 패턴을 파싱(`price-parser.mjs` 신규). SEOUL_YEYAK의 실제 원천 URL을
      직접 fetch해 확인한 결과 가격이 정적 HTML에 없어(동적 로딩 추정) 헤드리스
      브라우저 없이는 크롤링이 실제로 불가능함을 실측 확인 — 안 되는 걸 구현하는
      대신 정직하게 null 처리. `events.price_text`/`source_url` 컬럼 신규, 유저
      상세 화면(가격/홈페이지 행)에 노출.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-price-parsing-and-seoul-yeyak-reroute.md)
- [x] 개선사항9 (Step 115, Decision 024로 재승인 후 구현): 최초 스킵(Decision 017과
      충돌) 후 사용자가 "이거는 open_spaces말고.. 다시 이벤트 테이블로"라고 명시적
      재확인 → **Decision 024**로 Decision 017 2항 개정. SEOUL_YEYAK의 체육시설/
      공간시설을 open_spaces → events로 재라우팅(UI 카테고리 오매핑/키즈 뱃지 좁은
      판별 등 Decision 017 9항 취지는 그대로 유지). 기존 잘못 적재된 1,525건을
      raw_data로 재구성해 events로 이관 + 원본 삭제 완료(실측 검증됨).
      (완료: 2026-09-11, 상세: implementation/2026-09-11-price-parsing-and-seoul-yeyak-reroute.md)
- [x] 개선사항10 (Step 114): Event↔Spot 양방향 링킹. `events.space_id`(기존 FK
      컬럼, 신규 마이그레이션 불필요)를 재사용 — 프로덕션에서 좌표 30m 이내 +
      이름 부분일치 기준 배치 매칭으로 11,977건 자동 연결. 관리자 Events 탭
      상세 팝업에 "연결된 스팟" 편집기(SpotPicker 재사용) 추가. 유저 화면:
      이벤트 상세에 "📍 연결된 장소" 탭 가능 행, 스팟 상세에 "🎪 진행 중인
      이벤트" 목록(0건 시 숨김) — 둘 다 탭하면 DetailModal이 자기 자신을
      중첩 렌더링해 그 상세를 연다(부모 화면 15곳+ 수정 없이 자기완결적으로 구현).
      (완료: 2026-09-11, 상세: implementation/2026-09-11-event-spot-bidirectional-linking.md)
- [x] 개선사항7-2 후속 (Step 116, 사용자 추가 지시): "events쪽의 블로그 큐레이션에도
      가격이나 연령과 관련된 단어들을.. 노란색 형광펜 색칠해줘.. 블로그의 가독성을
      위하여 줄바꿈도.." — `highlightKeywordsOnly` 신규(카테고리 뱃지 없이 명시적
      키워드/패턴만 하이라이트), 가격 라벨/금액 키워드 세트 추가. `naver-blog-body.ts`의
      본문 추출이 `structuredText`의 문단 줄바꿈(`\n`)을 공백으로 뭉개던 버그 수정 +
      `whitespace-pre-line` 렌더링 적용.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-event-blog-curation-highlighting-and-readability.md)
- [x] 개선사항7-2 후속2 (Step 117, 사용자 추가 지시): "가격 무료인지.. 유료면 가격에
      대하여 블로그글 복붙하면 파싱할 수 있는거.. 적을 수 있는거 줘야지.. 타겟 연령..
      선택체크 할 수 있도록.. 아이/분수/빛/불꽃 등 키워드도 색칠해줘" — 기존
      `/api/admin/events/blog-curation` PUT을 확장해 `price_text`(자유 텍스트 직접
      입력 + "현재 블로그에서 자동 채우기" 버튼, `parse-price-from-text.ts` 신규:
      scripts/의 price-parser.mjs와 동일 로직을 TS로 중복 구현 — scripts/↔src/
      비상호참조 컨벤션)와 `target_audience`(EVENT_PICK_TARGET_AUDIENCES 4개 중
      단일 선택 토글 버튼, 기존 데이터그리드의 11개 값 TargetAudienceEditor와는
      별개의 좁은 목적)를 한 번에 저장. "건드리지 않은 필드는 PUT body에서 아예
      뺀다"(현재값==최초로드값이면 키 생략) 안전장치로 다른 화면에서 이미 검수한
      값을 실수로 덮어쓰지 않게 함. `KID_APPEAL_HINT_KEYWORDS` 신규(아이/분수/
      불꽃놀이/동화/공연/인형극/마술쇼/캐릭터/야경 등 + "아이들과 가기 좋은" 류
      문구) — "원" 같은 단일 글자·오탐 위험 단어는 제외.
      (완료: 2026-09-11, 상세: implementation/2026-09-11-event-curation-price-target-audience-kid-keywords.md)
- [x] 개선사항10 후속 (Step 118, 사용자 추가 지시): "이벤트픽에 스팟 연결하면 연결됨이라고
      뜨는데.. 해당 장소가 노출 중분류가 있는지 확인하고.. 없으면 노출 중분류를
      입력하라고 하고 해당 팝업화면에서 선택 및 저장 가능하도록 해줘" — 실측 확인
      결과 노출 중분류(service_category_id)가 없어도 필터 없는 기본 스팟픽 지도엔
      나오지만(location_precision=EXACT만 요구), 카테고리 필터(get_spots_by_service_
      category RPC)로는 전혀 걸리지 않음. `/api/admin/data-grid/space-link`에 GET
      추가(연결된 open_spaces의 name/service_category_id 조회), `SpaceLinkEditor`가
      스팟 연결/로드 시마다 이를 조회해 있으면 초록 배지, 없으면 경고 + 기존
      `ServiceCategoryEditor`와 동일한 선택 UI(저장은 기존 bulk-category-mapping
      POST 재사용)를 그 자리에서 보여준다.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-space-link-service-category-check.md)
- [x] 개선사항9 후속 (Step 119, 사용자 추가 지시): "시간관련 필드.. 예약 일자들도 항상
      최신값으로 갱신에 포함하는게 맞을거같은데... 주기적으로 바뀌어야 하는 필드에
      대하여 좀더 확인하고.. 최신값으로 갱신해" — 서울형 키즈카페 실측 조사 중 발견한
      버그: `upsertRowsSafeMerge`의 "기존 값 있으면 보존" 규칙이 events.start_date/
      end_date/reservation_start_date/reservation_end_date/is_active/booking_status
      처럼 회차가 바뀔 때마다 실제로 달라지는 필드에도 적용돼, 한 번 채워진 뒤로는
      새 예약 회차가 열려도 옛날 값에 영원히 고정되는 문제가 있었음(관악구 난곡동점 등
      대부분의 지점이 2026-08-25~09-07 회차에 고정, 실제 원본은 이미 09-10~09-21로
      넘어감). `ALWAYS_REFRESH_FIELDS` 신규 도입해 이 6개 필드만 예외적으로 "최신
      incoming이 항상 이김"으로 뒤집었다(category_min/target_audience 같은 관리자
      수동 분류 필드는 절대 포함하지 않음 — 회귀 방지 테스트로 확인). SEOUL_YEYAK
      1회 수동 재수집으로 기존 데이터도 즉시 반영.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-safe-merge-always-refresh-time-fields.md)
- [x] 개선사항3 후속 (Step 120, 사용자 버그 신고): "이벤트픽 전체보기 바텀시트에서
      리스트 누르면 상세카드 뜨는데, 취소하면 바로 전단계 리스트로 안 돌아가고
      그냥 사라져버리고 이벤트픽 화면이 뜬다.. 다시 전체보기부터 눌러야 함" — 원인:
      EventBrowseSheet(오늘 전체보기/알찬 픽/예약 픽 3곳)의 onSelectItem이 상세카드를
      열면서 동시에 시트 자체를 닫아버렸음(MajorCategoryGrid는 자체 로컬 상태라
      원래 문제 없었음). 시트를 닫지 않고 상세카드만 위에 겹쳐 띄우도록 수정 +
      "방금 누른 항목 포커스 표시"(파란 테두리 강조 + scrollIntoView) 신규 추가,
      전체보기 4곳 모두 일관 적용.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-eventpick-browse-sheet-detail-return-focus.md)
- [x] 개선사항10(맘스픽) 후속 (Step 121, 사용자 버그 신고): "맘스픽 들어가면 장소선택이
      같이 보여.. 첫글은 안쓴상태면 맘스픽 내용만 보여야돼.. 액션하면 그때 팝업떠서
      권한없다고 첫글쓰러 가자고 해야" — not_sprout_yet(로그인 O, 첫 글 X)이 guest와
      달리 실제 글쓰기 폼(SurveyReviewComposer, 1단계 장소선택)을 항상 그대로
      렌더링하고 있던 것을 guest와 동일한 소프트월 버튼으로 통일. 안내 팝업의
      "첫 글 쓰러 가기"를 눌러야만 폼이 드러난다(isComposerRevealed 신규).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-mompick-first-post-softwall-unify.md)
- [x] 개선사항10 후속2 (Step 122, 사용자 추가 지시): "이미 등록한적 있는데 일자만
      바뀌는 이벤트들(난지캠핑장/서울형 키즈카페 등).. 8월꺼에 스팟연결+노출
      중분류 해놨는데 9월꺼 가져왔으면 자동 매핑 가능한가?" — external_id가
      매달 새로 발급되는 소스(안내글마다 새 SVCID)는 새 이벤트 행이 매번 space_id
      NULL로 시작해 관리자가 매번 재연결해야 했음. Step 114의 일회성 매칭 SQL
      (좌표 30m+이름 부분일치, 멱등)을 `match_events_to_open_spaces()` RPC로 옮겨
      매일 배치 후처리 단계에 상시 편입. 노출 중분류는 스팟(open_spaces) 쪽 값이라
      space_id만 연결되면 자동으로 함께 적용됨. 프로덕션 1회 수동 실행으로 기존
      미연결 331건 즉시 정리(한성백제박물관점 9월 안내글 포함 확인).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-auto-match-events-to-open-spaces-daily.md)
- [x] 개선사항2 후속 (Step 123, 사용자 지시): "updated_at 어 이거 추가해.. 자동
      갱신 트리거도 하고" — "오늘 등록건"(created_at)만으로는 external_id가
      안정적인 소스가 오늘 실제로 갱신(예약 회차 변경 등)됐어도 절대 안 잡히는
      사각지대를 확인한 직후의 후속 요청. `events.updated_at` 컬럼 + 범용 트리거
      함수(`set_updated_at()`) 신규 — raw_data 등 잡음 컬럼은 비교에서 제외해
      "값이 하나도 안 바뀐 재적재"는 갱신으로 잡히지 않게 함(프로덕션 실측 검증
      완료). 요약 카드에 "오늘 갱신" 건수 추가, events 탭에 "수정일(updated_at)"
      필터 섹션(오늘 갱신건 보기/최근 3일) 신규 — 등록일 필터와 동시에 걸리면
      서로 좁혀버리는 함정을 막기 위해 한쪽을 켜면 다른 쪽을 항상 비운다.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-updated-at-column-and-filter.md)
- [x] 개선사항2 후속2 (Step 124, 사용자 버그 신고 + 검토 요청): "표준 중분류가
      미지정NULL 체크 된거는 활성 상태가 is_active true로 된게 안먹던데? 아주
      옛날 이벤트들도 다 나오고 있어.. 비활성 데이터 backup 테이블로 옮기는건
      어때? 검토해봐" — 실측 결과 필터 로직 버그가 아니라 데이터 정합성 버그였음:
      `deactivateExpiredEvents()`가 `.update().select('id')` 1회 호출만 해 PostgREST
      max_rows(1000)에 걸려 매일 최대 1000건만 비활성화되고 나머지는 누적, 실측
      시점 백로그 21,453건(전체의 75%, 2016년 종료건도 is_active=true로 방치).
      "id 소량 SELECT → 그 id로 UPDATE"를 0건까지 반복하는 방식으로 수정 +
      프로덕션 즉시 실행으로 백로그 전량 정리(잔여 0건 확인, active 24,789→3,336).
      백업 테이블 이전은 검토 결과 비권장(events 전체가 28,698건뿐이라 성능
      이점 미미, 오래된 건도 259~278건뿐 — 진짜 원인은 이 버그였음).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-deactivate-expired-events-backlog-fix.md)
- [x] 개선사항6 후속 (Step 125, 사용자 지시): "events 탭도 그리드 컬럼 축소해줘
      ID컬럼 숨겨줘.. DTLCONT 컬럼에 html로 안되어있는것들에 \r\n&nbsp;&nbsp; 이런식
      으로 되어있으면 정돈된 글로 볼수있게해줘.. html이 아닌경우 확인하고 정돈돼서
      볼수있게" — events 탭 ID 컬럼을 open_spaces와 동일하게 숨김(출처와 중복).
      `looksLikeHtml`(진짜 태그)과 상호 배타적인 `looksLikeMessyText`(개행/엔티티만
      섞인 경우) 신규 — "📄 정돈해서 보기" 버튼으로 \r\n을 실제 개행으로, &nbsp; 등
      엔티티를 브라우저 파서로 안전하게 디코딩해 정돈된 텍스트 팝업으로 보여준다.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-admin-grid-column-reduction-and-messy-text-cleanup.md)
- [x] 개선사항9 후속3 (Step 126, 사용자 지시 4건 연속): "공간시설/다목적실·회의실·
      주민공유공간·강의실.. 시설대관성 항목은 공공청사/행정으로.. 교육강좌/역사는
      배움/교육으로.. 전시실도 공공청사/행정으로.. 수영장·파크골프장은 체육시설로..
      지금 다 옮겨야 할것같아.. [+어드민 필터에] 전혀 반영안됐어 여기도 반영해줘" —
      seoul_public_reservation의 공간시설(1,256건)·체육시설(1,407건) 원천 대분류
      전체를 표준 대분류로 재정리(캠핑장만 예외로 자연/캠핑 유지), 교육강좌/역사
      78건만 배움/클래스로(나머지 2,506건의 문화행사성 "역사"는 성격이 달라 제외).
      전부 category_min_source='MANUAL'로 재분류 엔진 재실행에도 보호. 추가로
      DB 컬럼과 완전히 독립된 어드민 필터 정적 그룹핑(category-min-groups.ts)이
      실제 반영 안 되는 구조적 원인을 발견해 '전시실'을, 이어서 사용자 확인 후
      '공연장'/'광장'도 실제 코드에서 문화/축제·자연/체험 → 공공청사/행정으로
      이동("역사"는 대다수(96%)가 실제 문화 콘텐츠라 그룹은 그대로 둠). 매번
      DB(이미 반영돼 있는지)와 그룹핑 코드를 함께 재확인하는 동일 패턴 반복.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-category-reclassification.md)
- [x] 개선사항10 후속3 (Step 127, 사용자 지시): "스팟픽에서 연결된 이벤트 중에 저
      4개 연령(FAMILY/KIDS_PRE/KIDS_SCHOOL/INFANT) 아닌데 나오는게 있나 점검해봐..
      나오면 안되긴 해" — `/api/spots/linked-events`(스팟 상세 "🎪 진행 중인
      이벤트" 섹션)가 target_audience 필터가 전혀 없어 이벤트픽 피드엔 안 보이는
      이벤트도 그대로 노출되고 있었음(실측: 714건 중 515건, 72%가 4개 밖/NULL).
      다른 이벤트픽 조회와 동일하게 EVENT_PICK_TARGET_AUDIENCES 필터 추가.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-linked-events-target-audience-filter.md)
- [x] 개선사항9 후속4 (Step 126 후속3, 사용자 지시): "배움/교육 대분류에 있는
      교육체험에 대하여 대분류 자연/체험쪽으로 옮겨줘" — 앞선 3건과 달리 소스별
      성격 혼재가 아니라 순수 taxonomy 재분류 판단이라, 어드민 필터 그룹핑
      (category-min-groups.ts)만 이동하고 DB category_maj/공개 화면 taxonomy는
      건드리지 않음(임의 확장 금지).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-category-reclassification.md)
- [ ] (기획 메모, 2026-09-12 — 기준 확정, 구현은 "추후에") 스팟 선택 시
      프리뷰/상세카드에 "아직은 아니지만 진행중인 이벤트"를 연결 표시하는 기능.
      노출 기준 최종 확정(OR 조건, 둘 중 하나만 만족해도 노출):
      1. 현재일자가 이벤트 진행중 기간(start_date~end_date) 내 포함 AND
         타겟 연령이 4개(INFANT/KIDS_PRE/KIDS_SCHOOL/FAMILY) 중 하나
      2. 현재일자가 예약 진행중 기간(reservation_start_date~reservation_end_date)
         내 포함 AND 타겟 연령이 4개 중 하나
      "이 2개만 노출할꺼야 추후에" — 기준은 확정됐지만 사용자가 "추후에"라고
      명시해 지금 바로 구현은 시작하지 않는다. 착수 지시가 오면 시작.
- [x] 개선사항9 후속5 (Step 128, 사용자 지시): "events에서 표준 대분류가 문화/축제,
      자연/체험, 키즈/육아 안에 포함된 중분류만 사용할꺼야.. 15종만.. 나머지
      원천데이터 소스들은.. 사용안하는 중분류 쪽으로 넣지말고" — 실측 확인 결과
      매일 배치에서 실제로 도는 건 DB `category_rules` 테이블(events target)
      하나뿐(스크립트의 정적 CATEGORY_MIN_RULES는 2026-08-26 1회성 마이그레이션
      이후 어디서도 호출되지 않는 휴면 코드였음). 승인 15종 밖 6개 중분류(교양/어학·
      전문/자격증·정보통신·스포츠·청년정보·단체봉사)의 키워드 규칙 18건을 DB에서
      삭제, 이미 잘못 분류돼 있던 비-SEOUL_YEYAK 소스 325건을 미분류(NULL)로
      되돌림(MANUAL 지정 4건은 보호). seoul_public_reservation은 지시대로 원천
      대/중분류 그대로 유지(변경 없음). 코드 파일 변경 없이 전부 데이터/설정 작업.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-category-min-universe-restriction.md)
- [x] 개선사항9 후속6 (Step 129, 사용자 지시): "매일 재수집.. EVENTS탭.. 상세팝업..
      표준 중분류 수동선택하여 저장하는 부분엔 우리가 사용하기로 한 중분류들만
      나왔으면 해" — Step 128의 15종 정책을 관리자가 직접 고르는 CategoryMinEditor
      드롭다운에도 반영. `EVENTS_ALLOWED_CATEGORY_MINS`(기존 EVENTS_GROUPS_STATIC의
      3개 그룹에서 그대로 추출, 새 목록 하드코딩 안 함)를 events 탭에서만 적용,
      open_spaces는 무관. 현재 값이 15종 밖(SEOUL_YEYAK 강의실 등)이어도 값 자체는
      안 지움 — 새로 "고를 수 있는" 범위만 제한.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-category-min-manual-editor-restriction.md)
- [x] 개선사항10 후속4 (Step 130, 사용자 지시): "관리자 이벤트 상세 팝업 내 '운영
      요일 / 반복 규칙' 설정 추가.. 매일 운영(기본값) + 예외 규칙(주말만 운영/특정
      요일 지정/정기 휴무일 제외, 택1 또는 조합).. 이벤트 기간중이라도 이에 부합
      하지 않으면 안나오도록". `events.operating_weekdays`/`excluded_weekdays`
      text[] 컬럼 추가(둘 다 null/빈배열=제약없음, 독립 조합 가능 — 3개 토글이
      아니라 허용목록/제외목록 2개로 모델링). `isEventOperatingOn()` 순수함수로
      판정, 상세팝업(CategoryMinEditor/TargetAudienceEditor 다음 위치)에
      OperatingScheduleEditor 추가(프리셋 버튼 3종 + 정기휴무 체크박스, PATCH
      /api/admin/events/operating-schedule). 실제 필터링은 현재
      /api/spots/linked-events(스팟 상세→연결된 이벤트)에만 적용 — 이벤트픽
      홈피드(get-home-feed.ts) 등 다른 "오늘 진행중" 화면까지 확장할지는 사용자
      확인 필요(질문으로 별도 보고).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-operating-weekdays.md)
- [x] 개선사항10 후속5 (Step 130 후속, 사용자 지시): "스팟픽에 연결된 이벤트가 스팟픽
      화면의 스팟에서 어떤형태로든 뜨게 했을때 적용되어야하고 이벤트픽화면에서도
      뜨는 이벤트들에 대하여 해당 규칙대로 적용되게 해야돼" — 위 Step 130에서 남겨둔
      "확인 필요" 항목에 대한 답변. 스팟픽 쪽은 실측 확인 결과 연결된 이벤트가 뜨는
      화면이 detail-modal.tsx(스팟 상세의 "연결된 이벤트" 섹션, /api/spots/linked-events
      기반) 하나뿐이라 이미 적용 완료 상태(추가 조치 불필요). 이벤트픽 쪽은
      get-home-feed.ts의 EVENT_COLUMNS/EventRow에 operating_weekdays/excluded_weekdays를
      추가하고 `filterEventsOperatingToday()` 공용 헬퍼를 만들어 EVENT_COLUMNS를 쓰는
      이벤트픽 조회 함수 전부(오늘의 이벤트/현재 진행중/예약 가능/무료/테마/카테고리별
      피드 및 각 전체보기 페이지·검색, 총 10개 지점)에 공통 적용. /api/spots/linked-events도
      이 공용 헬퍼로 통합(중복 로직 제거). getCategoryMinCounts()(중분류 버튼 노출 여부를
      결정하는 COUNT 전용 쿼리)는 요일 배열 필터가 count(*) 헤드 쿼리로 표현되지 않아
      제외 — "그 중분류에 맞는 행이 존재하는지" 판단용이라 실제 카드 노출과는 무관.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-operating-weekdays.md)
- [x] 개선사항10 후속6 (Step 130 후속2, 사용자 지시): "매주 토요일 / 매월 2번째
      4번째 토요일 / 매주 주말 / 매주 월요일 휴무 / 매주 화, 목 운영 이런식의
      패턴이야 대부분" — 기존 operating_weekdays(매주 반복)만으로는 "매월 N번째
      요일" 패턴을 표현할 수 없어 `events.operating_nth_weekdays text[]`
      컬럼을 추가("N-요일코드" 토큰, 예: `{2-SAT,4-SAT}`). operating_weekdays와
      상호 배타적 대안 규칙으로 두되(있으면 우선), excluded_weekdays(정기 휴무)는
      어느 쪽과도 계속 조합 가능. `isEventOperatingOn()`에 월 기준 N번째 등장
      판정(`Math.ceil(date.getDate()/7)`) 로직 추가. 상세팝업에 "매월 특정 주차
      요일" 프리셋 추가(요일 선택 + 1~5주차 선택 → 조합 토큰 생성). 기존
      operating_weekdays/excluded_weekdays 필터 적용 지점(get-home-feed.ts 전체
      + linked-events)이 `filterEventsOperatingToday`/`OperatingSchedule` 타입을
      공유해 컬럼만 추가하면 자동 반영되도록 이미 설계돼 있어 별도 라우트 수정
      불필요.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-events-operating-weekdays.md)
- [x] 개선사항10 후속7 (Step 131, 사용자 지시): "스팟픽 상세카드쪽에 가격 영업시간
      메뉴 같은거 나오는거 이건 키즈친화 식당만 해당하는거 아냐?.. 상세카드
      구조는.. 크게 이 2가지(스팟 자체 상세카드 / 연결된 이벤트 상세카드)면
      되는지.. 검토해줘" — 검토 결과 detail-modal.tsx가 이미 item_type='EVENT'
      기준으로 완전히 분리된 2개 카드 구조를 갖고 있어(이벤트 카드엔 운영시간/
      메뉴 행 자체가 없음) 추가 구조 변경 불필요. 실제 문제는 스팟 카드 안에서
      "메뉴" 행이 카테고리 무관하게 전부 노출되는 것 — spot-curations-panel.tsx
      (관리자 큐레이션)가 애초에 키즈친화식당(category_min='놀이방식당')만
      후보로 검색하도록 하드코딩돼 있어 다른 카테고리는 메뉴를 영원히 입력할
      수 없는데도 "메뉴 정보는 순차적으로 추가될 예정" 문구가 계속 뜨고 있었음.
      사용자 확인: "키즈친화식당만(현행 유지)". `KIDS_RESTAURANT_CATEGORY_MIN`
      상수를 spot-category-groups.ts로 옮겨 관리자 큐레이션 후보 범위와 유저
      화면 메뉴 노출 범위를 단일 출처로 통일, detail-modal.tsx 메뉴 행에
      `item.category_min === KIDS_RESTAURANT_CATEGORY_MIN` 조건 추가(운영시간/
      가격은 다른 카테고리에도 유의미한 폴백이 있어 그대로 둠). 별도로 "요일별
      다른 주차 매칭"(예: 2번째 화요일+4번째 목요일 따로 지정) 확장은 보류
      확정("안되는걸로 하자") — 기존 구현(요일×주차 전체 조합만 지원) 유지.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-spot-detail-card-menu-scope.md)
- [x] 개선사항10 후속8 (Step 132, 사용자 지시): "운영 요일 / 반복 규칙으로 예외일자
      설정하는거.. 이거 관련 블로그 큐레이션 안으로 집어넣어줄수 있어? 보통
      RAW_DATA는 기간으로만 나와있어서.. 블로그 보고 파악하는데" — 원천 데이터는
      기간만 주고 실제 반복 패턴은 블로그를 읽어야 알 수 있다는 지적에 따라
      OperatingScheduleEditor를 상세 팝업의 독립 섹션에서 EventBlogCurationModal
      (블로그 큐레이션 모달) 안으로 옮김. raw-data-modal.tsx/
      event-blog-curation-modal.tsx 두 파일이 공유하는
      src/components/admin/operating-schedule-editor.tsx로 분리(AdminEventRow
      전체가 아니라 이 편집기가 실제 쓰는 필드만 요구하는 좁은 타입
      OperatingScheduleRow로 재사용성 확보). raw-data-modal.tsx의 관련 테스트는
      operating-schedule-editor.test.tsx로 이동, event-blog-curation-modal.test.tsx에
      내장 렌더링/저장 검증 3건 추가.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-operating-schedule-into-blog-curation.md)
- [x] Step 133 (사용자 버그 신고): "또 나드리픽 관리자화면 느려졌어 많이 느림..
      VACUUM을 하던 원인 확인하고 조치해줘 배큠해야하는지 인덱싱이 문제인지"
      — 실측 결과 open_spaces/events/raw_ingest_data 3개 테이블 모두 dead
      tuple 1~2%로 건강했음(2026-09-05/06 조치가 계속 유효, 이번엔 VACUUM
      문제 아님). 원인은 `get_events_filter_options()` RPC가 8초
      statement_timeout에 걸려 실패 — source/event_type/raw_data->>'MINCLASSNM'/
      'SVCSTATNM' 4개 값을 한 스캔에 몰아 집계하다 보니 raw_data(JSONB, 특히
      seoul_public_reservation 소스 평균 3.9KB·최대 82KB) TOAST 압축 해제
      비용이 매 행 발생. `raw_data->>'MINCLASSNM'`/`'SVCSTATNM'` 표현식
      인덱스 2개 추가 + 함수를 4개 독립 서브쿼리로 재작성해 8274ms(타임아웃)
      → 352ms로 해결. 코드 변경 없음(순수 DB 마이그레이션).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-admin-data-grid-events-filter-options-timeout-fix.md)
- [x] Step 134 (사용자 지시): "아까보단 빨라졌는데 그래도 느린데 db쪽이 아니라면
      프론트엔드쪽의 랜더링쪽에서 뭔가 느린거 아니야? 데이터를 가져오는게 크게
      없는데?" — 실측 결과 렌더링이 아니라 목록 조회 응답 자체가 화면에 안 보이는
      raw_data(JSONB)를 매번 통째로 실어 나르고 있었음. open_spaces는 raw_data를
      그리드 행이 전혀 안 쓰는데도 200건 조회 시 66.4KB(27%) 포함, events는
      3개 문자열 필드(MAXCLASSNM/MINCLASSNM/SVCSTATNM)만 쓰는데 404.3KB(62%)
      포함. open_spaces 목록에서 raw_data 완전 제거, events는 PostgREST 별칭
      (`max_class:raw_data->>MAXCLASSNM` 등)으로 필요한 3개 필드만 SELECT.
      상세 모달을 열 때만 신규 라우트(/api/admin/data-grid/raw-data)로 그 한
      건의 raw_data를 따로 받아옴(data-grid-client.tsx ensureRawDataLoaded).
      200건 기준 open_spaces 246.9KB→177.5KB(-28%), events 655.0KB→231.9KB
      (-65%).
      (완료: 2026-09-12, 상세: implementation/2026-09-12-admin-data-grid-raw-data-overfetch-fix.md)
- [x] Step 135 (사용자 지시): "관리자 화면에서 open_spaces쪽에 블로그 큐레이션도
      가져온 블로그 url에 대하여 선택하여 저장할 수 있게해줘.. events 탭처럼" —
      events는 검색 결과에 체크박스가 있어 고른 것만 저장하지만, open_spaces는
      체크박스 없이 검색된 상위 3개를 항상 자동 저장했다. /api/spot-blog-reviews
      확인 결과 spot_curations.blog_url_1/2/3은 이미 유저 화면 최우선 소스라
      DB 마이그레이션 불필요 — useSpotCurationForm에 selectedUrls/toggleUrl 추가
      (기본값 빈 배열, events와 동일), save()가 blogItems[0..2] 대신
      selectedUrls[0..2]를 저장하도록 변경. BlogCurationModal/
      MobileCurationWorkbench 둘 다(같은 훅 공유) EventBlogCurationModal과
      동일한 체크박스 목록(최대 3개) 추가. 동작 변화: 이제 아무것도 체크 안
      하면 아무 URL도 저장 안 됨(이전엔 자동 저장) — events와 동일하게 맞춘
      의도된 변경.
      (완료: 2026-09-12, 상세: implementation/2026-09-12-open-spaces-blog-curation-url-selection.md)
- [x] Step 136 (사용자 지시, 2026-09-13): "로그인 유저든 비로그인 유저든 맘스픽
      하단 버튼 눌러서 가면 맘스픽의 메인화면만 보여야돼.. 로그인하러가기라던가
      첫글쓰기 같은거 보이면 안돼.. 새싹맘 유저처럼 맘스픽 화면 동일하게
      보여야돼. 뭔가 누르려고 하면 그때 비로그인은 로그인 인증화면으로, 첫글
      안 쓴 유저는 첫글쓰기로 완전히 화면이 전환되어야해. 지금은 첫글쓰기할때
      아래 맘스픽 화면 메뉴가 같이나옴.. 공존이 아니고 완전히 전환되어야해." —
      guest/not_sprout_yet도 allowed와 동일하게 SurveyReviewComposer를 그대로
      렌더링(기존 소프트월 버튼 제거)하고, 그 위에 피드 영역과 동일한 투명
      인터셉트 레이어만 얹음. 실제 액션 시 MomPickView가 헤더/피드 없이 그
      화면만 반환하는 early return으로 전환(LoginPromptModal은 불투명 전체
      화면으로 변경, 첫글쓰기는 새 전체화면 상태로 전환) — RootLayout이 하단
      탭을 항상 렌더하지만 fixed inset-0가 시각적으로 덮고, 배경 DOM 자체가
      없어져 접근성/"완전 전환" 요구 모두 충족. 별도 라우트는 추가하지 않음.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-full-screen-transitions.md)
- [x] Step 137 (사용자 지시, 2026-09-13, Step 136 후속 정정): "여전히 맘스픽
      들어가면 어느 스팟인가요 장소 묻는거 뜨고 그아래 맘스픽 컨텐츠 뜨는데?
      이럼 안되지.. 맘스픽화면은 맘스픽 컨텐츠만 나와야지 왜 글쓰기 도입부가
      가장 위에 있어? 그리고 글쓰기쪽 내 주변 찾는거 엄청 느린데.. 힘들면
      이름으로 검색이 처음에 나오도록해" — Step 136에서 "새싹맘과 동일하게"를
      SurveyReviewComposer 인라인 노출로 구현한 게 오해였음이 확인됨. (1)
      메인 화면 상단의 작성 폼을 완전히 제거하고 상태 무관 동일한 "✍️ 글쓰기"
      버튼 하나로 대체(로그인 여부를 드러내지 않는 원칙은 유지) — 메인 화면엔
      이제 피드만 남음. allowed는 안내 모달 없이 버튼 클릭 즉시 전체 화면
      글쓰기로 전환. (2) 장소선택 1단계 기본값을 이름 검색(SpotPicker)으로
      변경(실측: get_nearby_spaces_and_events 30km 반경이 1~4초 걸림) — "내
      주변 인기 스팟" 목록은 토글로만 노출, 백그라운드 조회는 유지. RPC 자체
      최적화는 범위 밖(다른 화면과 공유하는 핵심 RPC라 위험도 높음, 사용자도
      조건부 대안을 제시).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-content-only-and-search-first-place-picker.md)
- [x] Step 138 (사용자 지시, 2026-09-13): "글쓰기는 맨 상단 말고 floating으로
      해서 우측하단에.. 글하나 썼고 새싹맘 됐는데 글쓰기 버튼이나 전체보기
      누르면 아직 새싹맘 아니에요 나옴.. 쓴 글이 인기 우수글에 바로 뜨는데
      무슨 기준?.. 글 한개가 차지하는 공간이 너무 커.. 사진은 사진 보기
      버튼으로.. 제목 라인 우측에 닉네임과 등급.. tag들도 너무 많네" — (1)
      글쓰기 버튼을 다른 화면 AI챗봇과 동일한 우측하단 FAB로 이동. (2) 버그
      수정: useMomPickAccess가 user 불변 시 프로필 재조회를 안 해 첫글
      작성 직후(grade 승급)에도 게이팅 state가 갱신 안 되던 문제 —
      refreshKey 옵션 인자 추가 + 글 등록 후 증가. (3) getTrendingPosts에
      like_count>0 조건 추가(좋아요 0인 방금 쓴 글이 동률 최신순으로 상위에
      뜨던 문제). (4) DashboardPostCard 컴팩트화: 제목줄에 닉네임/등급 통합,
      태그 최대 3개+"+N개", 사진 인라인 대신 "📷 사진 N장 보기" 버튼 +
      PostPhotoModal 팝업.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-fab-and-feed-fixes.md)
- [x] Step 139 (사용자 지시, 2026-09-13): "성남시 분당구에 사는 3살 3살 아이와..
      가 맘스픽 상단에 왜뜨지? 좀더 줄였으면.. 프리뷰카드라고 치고.. 내용글도
      1줄만.. tag 정도만.. 사진 1장 보기도 없애.. 프리뷰 카드 누르면 상세카드가
      보이게 되는 구조.. 찜도 없애.. 찜기능은 상세 카드쪽에서.. 일자도.. 굳이
      프리뷰에서 볼일은 없지 않나?" — (1) PersonalizedBanner(맞춤 환영 문구)를
      "맘스픽 컨텐츠 아님" 지적으로 받아들여 렌더링 제거(파일은 보존). (2)
      DashboardPostCard를 완전한 미리보기 전용으로 재정의: 자유글 line-clamp-1,
      날짜/좋아요/사진버튼 전부 제거, 카드 전체를 버튼화해 누르면 상세로 이동.
      (3) 신규 PostDetailModal: 날짜/전체 태그(날씨·인프라·동반형태 포함)/전체
      내용/사진 그리드(탭하면 PostPhotoModal이 그 인덱스부터 열림)/찜(좋아요)
      버튼을 전부 여기로 이동 — 좋아요는 기존 사장된 MomPickFeed가 갖고 있던
      toggleLike/getMyLikedPostIds를 재사용, Decision 019(열심맘 이상만 노출)
      규칙 유지.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-preview-detail-card-split.md)
- [x] Step 140 (사용자 지시, 2026-09-13): "1. 👑 파워맘 우수맘 추천 영역: 골드/퍼플
      톤, 헤더 '👑 이번 주 파워맘 추천 픽 + 전체보기 ›', 작성자 프로필 사진이
      큼직한 원형 아바타 중심 가로 카드, 터치 시 우수맘 큐레이션 리스트로 즉시
      이동. 2. 🔥 인기 우수글 영역: 오렌지/레드 톤, 헤더 '🔥 지금 가장 뜨거운
      인기 우수글 + 전체보기 ›', 썸네일+요약 텍스트 직사각형 컴팩트 가로 스크롤
      카드(좋아요 수 표시), 터치 시 인기글 전체 리스트로 이동. 3. 🔴 실시간
      라이브 영역: 레드/그린 톤, 헤더 '🔴 실시간 라이브 현장 + 전체보기 ›',
      반짝이는 라이브 뱃지의 타임라인형 가로 카드, 터치 시 실시간 전체 목록으로
      이동. 일단 이렇게 해줄수 있어?" — 3개 섹션 전용 카드/헤더를 새로 만들어
      맘스픽 메인 화면(미리보기)에 적용했다. 기존 공용 PreviewSection +
      DashboardPostCard(클릭→PostDetailModal) 조합을 대체해, 이제 미리보기
      카드를 누르면 개별 글 상세가 아니라 해당 섹션의 "전체보기" 목록 화면으로
      바로 이동한다(전체보기 페이지 자체는 기존 DashboardPostCard를 그대로 써서
      클릭→상세 모달 동작을 유지). [정직한 데이터 한계] profiles 테이블에
      프로필 사진(avatar_url) 컬럼이 없어(실측 확인, 업로드 기능도 스펙에
      없음) 파워맘 카드의 "프로필 사진"은 닉네임 첫 글자 아바타로 대체했고,
      "실시간 라이브"의 "진행 중인 이벤트"는 기존 getLivePosts()(단순 최신순
      게시글 피드, 2026-09-02 기획 그대로) 데이터 위에 상대 시간(N분/시간전)
      표시만 새로 얹었다 — 새 이벤트 상태나 조회수 컬럼을 임의로 만들지 않았다
      (제3장 제5조 추측 금지).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-three-section-redesign.md)
- [x] Step 141 (사용자 지시, 2026-09-13): "지금 음.. 글이없어.. 근데 그래서
      그런지 3가지 영역 다 상단으로 올라가 있는데? 글이 없어도 세 영역이
      화면에 대하여 상단 중단 하단으로 영역 분리되어서 고정으로 되었으면
      좋겠음. 그리고 실시간 라이브 현장 좋은데.. 하린맘 이건 빼고.. 제목하고
      3시간 전 이라는 시간하고.. 내용글? 조금? 그 옆에 남는 공간만.. 내용으로
      채우는거.. 한줄로 돼?" — (1) 3개 섹션(ExpertPickSection/
      TrendingPickSection/LivePickSection) 각각에 `min-h-[28vh]`를 부여하고,
      글이 없을 때 안내 문구를 그 영역 안에서 세로 중앙 정렬(`flex-1 items-
      center justify-center`)해, 글이 하나도 없어도 3개 영역이 화면 상/중/하로
      고정 분리돼 보이게 함(글이 많으면 자연스럽게 더 늘어남). (2)
      LivePickCard에서 작성자 닉네임 줄을 완전히 제거하고, LIVE 뱃지+제목+
      시간(고정 너비)과 내용글(남는 공간을 채우는 flex-1 + truncate)을 전부
      한 줄에 배치.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-fixed-zones-and-live-card-oneline.md)
- [x] Step 142 (사용자 지시, 2026-09-13): "3분할 좋은데.. 일단 좀 영역이
      전반적으로 내려갔다해야하나? 아래.. 하단 버튼 맘스픽/스팟픽/이벤트픽/
      마이 이거 버튼있는 영역을 고려를 안했어.. 이만큼 높이 빼고 3분할
      하던가 해야지. 그리고 각 영역별 좀 구분이 되도록 가로줄 하나라도
      놓던가.. 각 영역마다.. 아니면 색깔로 구분하던가.. 일단 제목이 있긴
      한데.. 그리고 실시간 라이브 현장은 가장 최신글이 맨 위로 가도록
      해야하나? 시간순으로?" — (1) min-h-[28vh](뷰포트 전체 기준, 하단
      탭바 높이를 반영 못함 — data-grid-client.tsx에 이미 기록된 동일한
      vh 문제)를 flex-1로 교체해, mom-pick-view.tsx 최상위 wrapper부터
      3개 섹션까지 flex-1을 그대로 이어 붙임으로써 "헤더/하단 탭바를 뺀
      실제 남은 화면 높이"를 3등분하도록 수정. (2) 3개 섹션 각각에 테마
      색상 배경 패널(rounded-2xl border + 그라데이션/틴트 배경 — 파워맘:
      골드/퍼플, 인기글: 오렌지, 라이브: 레드)을 둘러 영역 경계를 색으로
      구분(테두리가 구분선 역할도 겸함). (3) 실시간 라이브 정렬 확인: 이미
      getLivePosts()가 created_at 내림차순(최신순)으로 조회하고 있어 별도
      코드 수정 없이 요구사항을 충족하는 상태임을 확인.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-real-height-split-and-section-tint.md)
- [x] Step 143 (사용자 지시, 2026-09-13): "맘스픽의 상세카드까지 넘어갔으면
      상세카드에서 그 이장소가 궁금하신가요? 문구랑 해당 장소 넘어가기?
      가보기? 이런거 버튼넣는게 나으려나 아니면 장소 옆에 해당 스팟 가는 거
      넣는게 좋으려나? 어떤형식이 더 좋을까? 더 나아보이는 방식으로 좀
      추천해줘 어쨌든 맘스픽으로부터 스팟픽의 해당 장소로 갈수 있어야해..
      상세카드내에 그게 있어야해" — 두 형식 중, 지도 화면(detail-modal.tsx)에
      이미 있던 "📍 연결된 장소: ... ›" 행 버튼과 같은 컴팩트한 행 형식을
      추천·채택(스크롤 없이 항상 보이고 기존 앱 톤과 일치, 헤더에 스팟명이
      이미 있어 "궁금하신가요?" 블록은 정보 중복). 구현: (1) mom_pick_posts.
      spot_id(최초 테이블 생성 시부터 있던 컬럼)를 DashboardPost.spotId로
      노출. (2) 신규 GET /api/spots/by-id?id=<uuid> — 기존 /api/events/
      linked-spot과 동일하게 SPACE_COLUMNS/toSpaceItem 재사용해 단건 조회.
      (3) PostDetailModal: spotId가 있으면 "📍 스팟픽에서 이 장소 보기 ›"
      링크(/nearby?spot=<id>)를 헤더 바로 아래에 노출. (4) map-explorer.tsx:
      ?spot=<id> 쿼리 파라미터를 읽어 그 스팟을 조회 후 곧장 전체 상세
      (DetailModal)를 자동으로 연다(개선사항10의 "Event➔Spot" 이동과 같은
      목적, 페이지 간 이동이라 쿼리 파라미터로 받는다는 점만 다름).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-to-spot-navigation.md)
- [x] Step 144 (사용자 지시, 2026-09-13): "어.. 닫았을때도 그 스팟을 지도에
      임시로라도 마커로 보여줘.. 얘가 스팟픽 벗어날때까지는.. 다음에 다시
      진입시는 안보이겠지. 그리고 또 뭐가 있어야 할까? 맘스픽의 우수글이라던가는
      보이도록 해야하나? 해당 스팟에 대하여? 아니면 관리자한테 해당 장소에
      대하여 글이 달렸는데 노출중분류 안되었다고 해서 달라고... 뭔가 알림?
      아니면 관리자화면에 별도로 보여야하나? 그래서 관리자가 해당 장소에
      대하여 맘스픽에서 쓴글이랑 뭐 블로그 큐레이션 가지고 확인하고... 확인하고
      노출중분류 추가하던가" — (1) 구현 완료: map-explorer.tsx에 linkedSpotItem
      state를 추가해, "/nearby?spot=<id>"로 들어와 상세 카드를 닫아도 그 스팟이
      제휴 스팟(2026-09-10, 노출 중분류 무관 노출)과 동일한 mergeById 관례로
      지도 마커에 계속 남도록 함(MARKER_LIMIT로 잘려도 살아남게 배열 맨 앞에
      둠). 컴포넌트 로컬 state라 /nearby를 벗어나면(다른 탭 이동=언마운트)
      자동으로 사라진다. (2) 열린 질문 — 구현하지 않고 추천만 제시, 사용자
      결정 대기 중: "맘스픽 우수글을 스팟 상세에 보여주기" vs "관리자 화면에
      '노출 중분류 미지정 + 맘스픽 글 있음' 우선순위 목록"(admin/category-
      mapping-panel.tsx의 기존 RowPicker가 이미 "노출 중분류 미지정만 조회"
      기능을 갖고 있어, 여기에 "맘스픽 글 존재 여부" 필터/정렬만 얹으면 되는
      구조 — 완전히 새로 만들 필요 없음) 중 후자(관리자 우선순위 큐)를
      추천했다 — 근본 원인(노출 중분류 누락)을 해결해 모든 사용자에게 정상
      노출되게 하는 것이 딥링크로 임시 우회하는 것보다 가치가 크다고 판단.
      사용자 응답 대기 중이며, 다음 세션은 여기서 이어서 확인할 것.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-mom-pick-linked-spot-temp-marker.md)
- [x] Step 145 (사용자 지시, 2026-09-13): "어 구현해줘.. 관리자 쪽.. 맘스픽에
      대하여 노출 중분류가 없는 건 별도의 관리자화면의 탭에서 맘스픽글
      올라왔고 이게 장소연결됐는데 장소에 대한 노출중분류가 안되어 있다..
      그리고 해당탭에 그런게 수시로 올라올수 있으니 탭 자체에 1건이라도
      있을경우 탭에 표시...해줘 그럼 아 그 표시 보고 관리자가 새로 들어온글이
      있고 노출중분류 안된장소가 있나보다 파악하고 그 탭 열고 작업하지" —
      직전 Step 144에서 추천했던 "관리자 우선순위 큐"를 구현했다. (1) 신규
      GET /api/admin/mom-pick-unmapped-spots: mom_pick_posts.spot_id가 있는
      글 중, 연결된 open_spaces.service_category_id가 아직 null인 스팟만
      골라 스팟 정보+글 목록(작성자 닉네임 포함)+기존 spot_curations 블로그
      큐레이션을 함께 반환(mom-pick-dashboard.ts의 "여러 쿼리 후 JS에서
      이어붙이기" 관례 재사용, PostgREST 임베디드 null 필터는 검증된 선례가
      없어 추측으로 쓰지 않음). (2) 신규 8번째 관리자 탭 "🚩 노출 중분류
      필요"(MomPickUnmappedSpotsPanel) — 다른 자기완결 패널과 달리 마운트
      즉시 자동 조회(배지의 의미를 살리기 위한 의도적 예외), 스팟별로 글/
      블로그 큐레이션을 보여주고 "노출 중분류 지정하기"를 누르면 기존
      MobileCurationWorkbench(category-mapping-panel.tsx와 동일)를 그대로
      재사용해 큐레이션+노출 중분류 지정까지 한 화면에서 처리. (3) 탭 배지:
      AdminDataGridClient 최상위에서 마운트 시 한 번 개수를 조회해 1건
      이상이면 탭 버튼에 빨간 숫자 배지를 표시(다른 패널의 "탭을 열어야
      조회" 관례의 예외 — 배지는 탭을 열기 전에 보여야 의미가 있음).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-admin-mom-pick-unmapped-spots-tab.md)
- [x] Step 146 (사용자 지시, 2026-09-13): "음 그런데.. 이거 블로그 큐레이션이
      위에 있고 뱃지 다는게 아래있는건 안되나? 그리고 노출중분류 먼저 지정
      저장한후에.. 블로그 큐레이션으로 불러와야 키워드가 매핑되어서 노란색
      포인트 주는데..." — MobileCurationWorkbench(category-mapping-panel.tsx/
      mom-pick-unmapped-spots-panel.tsx 공용)의 "2. 노출 중분류 & 편의시설
      뱃지"/"3. 블로그 참고" 두 섹션 순서를 뒤집어 블로그 참고를 "1."로,
      노출 중분류를 "2."로 옮겼다. 코드를 확인한 결과 하이라이팅(노란색
      키워드 포인트)은 curationCategoryId가 저장 여부와 무관한 현재 드롭다운
      선택값(serviceCategoryId, 메모리상 state)에서 매 렌더 계산되고
      BlogReferenceViewer의 하이라이트 함수도 useMemo 없이 매 렌더 순수
      계산돼, "저장까지 해야 매핑된다"는 전제 자체가 사실이 아니었다 — 순서를
      바꿔도(저장 없이 선택만 해도) 이미 불러온 블로그 본문이 즉시
      재하이라이트된다는 것을 코드로 확인하고 그대로 반영했다.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-curation-workbench-section-reorder.md)
- [x] Step 147 (사용자 지시, 2026-09-13): "네이버 예약 링크를 걸려고 해도..
      이게 url 안주어지는데? 안으로 숨긴거 아니야?" — 실측 확인: 네이버
      블로그 검색 API(search/v1/blog)는 블로그 글만 검색하므로 예약 페이지
      링크는 애초에 검색 결과로 나올 수 없고, 기존 "다른 URL로 바꾸기"
      수동 입력 UI는 이미 있는 검색 결과 슬롯이 있을 때만(activeItem 존재)
      나타나도록 짜여 있어 검색 결과가 0건이면 관리자가 URL을 입력할 곳
      자체가 화면에 없었다(숨긴 게 아니라 렌더 조건 문제). (1)
      use-spot-curation-form.ts의 overrideActiveUrl 가드를 완화해 검색 결과가
      0건이어도(빈 배열) 새 슬롯을 만들 수 있게 하고, 성공 시 hasNoResults를
      false로 되돌림. (2) blog-reference-viewer.tsx에 결과 0건일 때도 보이는
      "🔗 URL 직접 추가하기" 버튼+입력 UI를 추가. BlogCurationModal/
      EventBlogCurationModal/MobileCurationWorkbench 3곳 전부 이 공용
      컴포넌트를 쓰므로 한 번의 수정으로 전부 반영됨.
      (완료: 2026-09-13, 상세: implementation/2026-09-13-blog-curation-manual-url-when-no-results.md)
- [x] Step 148 (사용자 지시, 2026-09-13): "어 왜 쿼리가 타임아웃나는지.. 원인
      진단해서 고쳐줘. 그리고 큐레이션/제휴 상품 등록탭에서도 장소 입력하면
      해당 장소가 노출 중분류 없으면 관리자가 입력할수있도록 노출중분류도
      뜨게 해줘 events쪽 의 관리자 상세 팝업에서 장소 넣으면 노출중분류
      매핑되어있는지 체크해서 없으면 넣으라고해주는것처럼." 두 가지 작업.
      (1) events upsert statement timeout 진단/수정: Supabase Management
      API로 직접 실측 — events.updated_at 자동 갱신 트리거(2026-09-12,
      전체 컬럼 jsonb 비교)와 trigram 인덱스 3종(description 29MB 포함)
      유지 비용이 겹치고, SEOUL_YEYAK이 run-daily.mjs STEPS 순서상 대량
      upsert(SEOUL_CULTURE_EVENTS 18000+건) 직후 마지막에 실행돼 DB가
      부하 받은 상태에서 시작하는 게 겹쳐 간헐적으로 2분 statement_timeout을
      넘김을 확인(pg_stat_user_tables: n_tup_upd 108만 vs n_live_tup 3.3만
      — 매일 테이블 전체를 다시 쓰는 패턴). 가장 안전한 완화책으로
      upsertRowsSafeMerge()의 UPSERT 배치 크기를 500 → 200(기존
      SELECT_LOOKUP_BATCH_SIZE와 동일)으로 낮춰 단일 SQL 문에 실리는 트리거/
      인덱스 비용 자체를 줄임(스키마 변경 없음, upsertRows()가 쓰는 나머지
      25개 어댑터는 영향 없음). (2) 큐레이션/제휴 상품 등록 폼(curated-item-
      form-modal.tsx)에 스팟 연동 시 노출 중분류 확인/입력 UI 추가 — 기존
      events 탭 상세 팝업(raw-data-modal.tsx의 SpaceLinkEditor)에 있던 로직을
      SpotServiceCategoryCheck로 뽑아 공유(제5장 제4조).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-events-upsert-timeout-fix.md,
      implementation/2026-09-13-curated-items-service-category-check.md)
- [x] Step 149 (사용자 지시, 2026-09-13): "안쓰이는 인덱스에 대하여 정리하게
      인덱스의 여태까지 사용 비율같은거랑 어디에 사용되는지를 조사해서
      알려줘" → "어 그래 2개 삭제해줘". events 테이블 인덱스 21개 전수를
      pg_stat_user_indexes 실측 + 코드 전수 조사로 대조해 보고한 뒤, 사용자가
      승인한 2개를 삭제했다. (1) idx_events_description_trgm(29MB, idx_scan=2,
      idx_tup_fetch=0 — 21개 중 가장 크면서 가장 안 쓰임, 2026-09-13 events
      upsert statement timeout의 원인 중 하나로 이미 지목됨). (2)
      idx_events_category_maj(672KB, idx_scan=10 — category_maj 컬럼은
      코드 전체에서 INSERT/표시에만 쓰이고 .eq()/.in() 등 필터·정렬 조건으로
      쓰이는 곳이 전무함을 grep 전수 확인). 마이그레이션 파일 작성 후
      Supabase Management API로 프로덕션에 직접 적용, pg_indexes 재조회로
      실제 삭제 확인. idx_events_target_audience/idx_events_reservation은
      "애매한 후보"로 보고만 하고 삭제하지 않음(사용자가 이 둘은 승인하지
      않았음).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-drop-unused-events-indexes.md)
- [x] Step 150 (사용자 지시, 2026-09-13): "어 수집안된거 재수집은 했어
      오늘꺼?" → "오늘 놓친거 해줘 재수집". 오늘 두 번 실패했던 SEOUL_YEYAK
      소스를 `node scripts/ingest/run-daily.mjs --only=SEOUL_YEYAK`로 수동
      재실행했다. Step 148에서 적용한 배치 크기 축소(500→200) 이후 첫 실제
      실행이었는데, "canceling statement due to statement timeout" 에러 없이
      깨끗하게 성공(events 2572건 적재, 에러 0건, 드롭 0건 확인) — 타임아웃
      완화 조치가 실제로 효과가 있었음을 확인했다. docs/pipeline-log.md에
      자동 기록된 실행 로그를 그대로 커밋.
      (완료: 2026-09-13, 별도 implementation 기록 없음 — docs/pipeline-log.md
      자체가 실행 기록이며, 원인 진단/수정 내용은 Step 148에 이미 기록됨)
- [x] Step 151 (사용자 지시, 2026-09-13): "어 원인 진단 및 조치해" — 관리자
      화면 운영 가이드 캡처 중 발견된 `get_events_filter_options 조회 실패:
      canceling statement due to statement timeout`(Step 133에서 이미 한 번
      고쳤던 RPC가 재발) 진단/수정. 실측(EXPLAIN ANALYZE, pg_roles): raw_data
      ->>'MINCLASSNM'/'SVCSTATNM' 서브쿼리 2개가 array_agg(distinct.. order by)
      정렬 비용까지 겹쳐 최대 3초 이상 걸렸고, 4개 서브쿼리 합산 시간이
      authenticated/anon 롤의 실제 statement_timeout(8초 — Management API
      세션의 2분과는 다른 값임을 실측으로 처음 확인)을 넘을 수 있었다. 이
      두 필드는 SEOUL_YEYAK 소스에만 있어 그 소스가 매일 대량 재적재될 때마다
      재현 가능. get_sigungu_options()를 고쳤던 것과 동일한 근본 원인(요청마다
      재집계할 필요 없는 참조성 데이터)이라 동일한 해법(머티리얼라이즈드 뷰
      캐싱)을 그대로 적용 — events_filter_options_cache 뷰 생성, 함수는
      캐시를 읽기만 하도록 재정의(시그니처 동일이라 프런트엔드 무변경),
      run-daily.mjs에 REFRESH_EVENTS_FILTER_OPTIONS_CACHE 후처리 단계 추가.
      실측 검증: 조회 시간이 4.7ms 수준으로 즉시 응답하도록 개선(수 초~타임아웃
      → 0.1ms대).
      (완료: 2026-09-13, 상세: implementation/2026-09-13-events-filter-options-timeout-recurrence-fix.md)
- [x] Step 152 (사용자 지시, 2026-09-14): "어 최종적으로 얼굴 ㄹ인식해서
      얼굴에 스티커 합성해줘.. 이에 대하여 사용자가 최종 판단하고 .. 클릭으로
      이거 얼굴이고 이거 얼굴아니다도 수동으로 할 수 있도록 하는게 나을꺼같은데.."
      (직전 "검토해줘" 요청에 대한 기술 검토 답변 후 실제 지시). 맘스픽 글쓰기
      3단계 사진 업로드에 얼굴 스티커 편집 단계를 추가 — @tensorflow/tfjs +
      @tensorflow-models/blazeface로 클라이언트에서 얼굴을 자동 인식해 스티커를
      기본 적용하되, 사용자가 캔버스를 클릭해 스티커를 제거("얼굴 아니다")하거나
      빈 자리에 추가("얼굴이다")할 수 있게 해 최종 판단은 항상 사용자가 하도록
      설계. 신규 face-sticker-editor.tsx 모달, survey-review-composer.tsx는
      즉시 업로드 대신 편집 큐를 거치도록 수정. Playwright로 실제 브라우저에서
      자동 인식→수동 추가/제거→완료(Blob 생성)까지 전체 흐름 확인.
      (완료: 2026-09-14, 상세: implementation/2026-09-14-face-sticker-editor.md)
- [x] Step 153 (사용자 지시, 2026-09-14): "어 이상한데 일단 글쓰기해서 사진
      올리니깐 사진이 엄청 길어지더라? 취소버튼도 안보여... 그래서 뒤로가기
      누르니깐 그 사진팝업만 없어지는게 아니고 완전히 빠져나오던데 ? 이벤트픽
      으로 갔나 맘스픽 메인화면으로 갔나.." — Step 152 얼굴 스티커 편집기의
      실사용 버그 리포트. 재현해보니 표시 캔버스 크기 계산이 가로 폭만
      제한하고 세로 높이는 제한하지 않아, 세로로 긴 휴대폰 사진일수록 모달이
      화면보다 훨씬 길어져 "완료"/"취소" 버튼이 화면 밖으로 밀려나 안 보였고,
      사용자가 다른 탈출 수단이 없어 브라우저/앱 뒤로가기로 글쓰기 화면
      전체를 빠져나가게 된 것이 원인이었다(팝업만 닫는 로직 자체가 없었던
      게 아니라 버튼이 안 보였을 뿐). 가로/세로 모두 제한하도록 수정하고,
      모달 카드에 최대 높이+내부 스크롤을 둬 버튼 줄이 항상 화면 하단에
      고정되어 보이게 했으며, 어두운 배경 클릭으로도 취소되는 표준 탈출
      경로를 추가했다. 1080×1920 세로 사진 + 390×700 휴대폰 뷰포트로
      Playwright 재현 검증.
      (완료: 2026-09-14, 상세: implementation/2026-09-14-face-sticker-editor.md
      "후속 수정" 항목)
- [x] Step 154 (사용자 지시, 2026-09-14): "어 근데 뒤로가기도 이벤트픽으로
      빠져나가는게 아니고 .. 사진 연거 닫는걸로 해주면 안돼? 그게
      사용자경험 해치나?" 및 "그렇군 그래.. 사진이 근데 왜이렇게 길지
      가로길이가 ? 그거랑 .. 사이즈조절이 자동안되는거 같은데 ? 그거랑
      처음에 자동인식으로 가려지는거 안되는데 ?" — 두 가지.
      (1) 뒤로가기 가로채기 요청은 위험을 설명하고 보류하기로 합의:
      `use-modal-back-close.ts`(history.pushState 기반)를 React
      onClick/onChange로 여는 모달에 적용했다가 Next.js App Router 자체
      라우팅 감지와 충돌해 모달이 렌더링되지 않거나 페이지가 리로드되는
      버그가 이미 실측 발견되어 되돌려진 선례가 두 건(detail-modal.tsx,
      ai-recommend-sheet.tsx, 2026-08-29) 있어 동일 위험군인
      FaceStickerEditor에는 적용하지 않기로 함(코드 변경 없음, 버튼 가시성
      수정으로 실질적 필요성은 이미 해소됨).
      (2) 사진이 찌그러져 보이고 자동 얼굴 인식이 잘 안 되는 문제는 실제
      버그로 확인 후 수정: 캔버스를 고정 px 두 값으로 그리면서 CSS
      max-w-full이 폭만 줄이고 높이는 그대로 남아 세로 사진이 찌그러져
      보였고, 직전 수정에서 표시 캔버스 자체를 380px로 줄인 게 얼굴 인식
      입력 해상도도 함께 줄여 인식률을 떨어뜨렸다. 내부 렌더링/인식
      해상도(긴 변 480px)와 화면 표시 크기(최대 300×420px, JS로 직접 계산한
      고정 px)를 분리해 해결. Playwright로 세로/가로 이미지 각각 렌더링
      비율이 실제 비율과 일치함을 실측 검증.
      (완료: 2026-09-14, 상세: implementation/2026-09-14-face-sticker-editor.md
      "후속 수정 2차" 및 "뒤로가기 관련" 항목)
- [x] Step 155 (사용자 지시, 2026-09-14): "사진 올리려고 사진집찾아보고
      하는데 어느순간 닫으면 다시 첫글 쓰기? 처음으로 넘어와 있음... 왜그런지
      모르겠네... 그리고 이거 제목이 왜 첫글쓰기야 ? 난 여러번째 글쓰기
      중인데.." — 두 가지 리포트 중 확정 가능한 것만 이번에 수정.
      (1) 제목 오표기(확정 버그, 수정 완료): 전체화면 글쓰기 헤더가
      "✍️ 첫 글 쓰기"로 고정되어 있었는데, 이 화면은 새싹맘 미달성 유저의
      "첫 글 쓰러 가기" 경로뿐 아니라 이미 여러 번 글을 쓴 유저의 평소
      "✍️ 글쓰기" 플로팅 버튼으로도 똑같이 열린다 — "✍️ 글쓰기"로 일반화.
      (2) "사진 고르다 닫으면 1단계로 리셋" 이슈는 코드 리뷰로 원인을
      확정하지 못해 이번엔 수정하지 않음: `SurveyReviewComposer`가 다시
      마운트될 경로는 `isFullScreenComposerOpen`이 false→true로 바뀌는
      경우뿐이고 파일 선택 자체는 이 상태를 건드리지 않는다. 유력한 가설
      두 가지(①모바일 브라우저가 네이티브 사진 선택 UI 체류 중 메모리 회수
      목적으로 웹뷰를 재로드하는 OS 동작 — JS로 방지 불가, ②전체화면
      글쓰기 자체가 원래부터 뒤로가기/네비게이션 이탈 보호가 없었음 — 이번
      세션에서 다룬 얼굴 스티커 모달과는 별개의, 더 상위 레벨의 기존
      갭)를 implementation 기록에 남기고, Playwright로는 네이티브 OS 파일
      선택기를 재현할 수 없어 사용자에게 기기/브라우저 정보를 요청함.
      (완료: 2026-09-14, 상세: implementation/2026-09-14-write-screen-title-fix.md)
- [x] Step 156 (사용자 지시, 2026-09-14): "안드로이드 크롬" — Step 155에서
      기기/브라우저 정보를 요청한 데 대한 답변. 안드로이드 크롬은 메모리
      압박 시 백그라운드 탭을 통째로 버리고(discard) 돌아왔을 때 페이지를
      처음부터 재로딩하는 known 동작이 있어(모든 React 상태 소실, 증상과
      일치) 가설을 좁혔다. 이 기회에 실제 코드 원인 하나를 찾아 고침:
      `@tensorflow-models/blazeface`의 모델 객체는 내부 GraphModel이
      private라 공개 dispose() 메서드가 없어 GPU 메모리를 직접 해제할
      방법이 없는데, 기존 코드는 사진을 편집할 때마다(큐에 여러 장 있으면
      장마다) `blazeface.load()`를 새로 호출해 해제 불가능한 모델
      인스턴스가 사진 수만큼 계속 쌓이는 구조였다 — 메모리 압박을 스스로
      키워 안드로이드의 탭 discard 확률을 높이고 있었을 가능성. 모듈
      레벨에 모델 로딩 Promise를 캐시해 세션당 한 번만 로드하고 이후
      모든 사진 편집이 같은 인스턴스를 재사용하도록 수정. Playwright로
      사진 2장 연속 편집 시 모델 관련 네트워크 요청이 첫 장에서만
      발생함을 실측 검증(단, OS 차원의 탭 discard 자체를 완전히 없애는
      근본 해결은 아니고 확률을 낮추는 완화책이라는 한계를 명시).
      (완료: 2026-09-14, 상세: implementation/2026-09-14-write-screen-title-fix.md
      "사진 고르다 닫으면 처음으로 돌아감 이슈 후속" 항목)
- [x] Step 157 (사용자 지시, 2026-09-14): "지금 글쓰기할때 반응이 너무
      느린데 ? 처음에 장소찾는것도 엄청느리고 사진추가때 사진 선택하고
      완료버튼 누를때도 반응속도가 너무 늦어 안된건가 싶어서 2번이나 누르고
      그러니 2번이나 선택되네.. 전반ㄱ적으로 앱자체가 너무 느린거 같은데
      왜그런지 원인 파악하고 조치해" — 세 증상 각각 실측으로 원인 확인 후 조치.
      (1) 장소 검색: pg_trgm GIN 인덱스는 3글자 미만 토큰에서는 완전한
      trigram을 만들 수 없어 인덱스가 사실상 무용지물이 되는데, 프런트엔드는
      검색어 전체 길이만 3자 이상인지 검사해 "행복 어린이집"처럼 짧은
      단어가 섞인 자연스러운 검색이 그대로 통과되고 있었다. 운영 DB에 직접
      EXPLAIN ANALYZE 실행해 8,009ms 걸리던 쿼리를 재현/확인, 3자 미만
      토큰을 걸러내는 selectTrigramFriendlyTokens()를 추가해 99.9ms로
      개선(80배) — searchSpacesNationwide/searchEvents 양쪽에 적용.
      (2) 사진 "완료" 버튼: 원본 해상도(휴대폰 사진 수 MB) 그대로 인코딩+
      업로드하고 있어 느렸고, 처리 중 버튼 비활성화가 없어 중복 클릭 시
      같은 사진이 두 번 업로드되는 버그를 코드로 확인. 업로드 전 긴 변
      1600px로 다운스케일, 완료/건너뛰기/취소 버튼 전체를 처리 중 잠그도록
      수정.
      (완료: 2026-09-14, 상세: implementation/2026-09-14-write-flow-performance.md)
- [x] Step 158 (사용자 지시, 2026-09-15): "1300 / 1400px 등도 되는거야 ?
      1400px 나아?" — Step 157에서 적용한 업로드 이미지 다운스케일 상한
      (1600px)에 대해 "업계 표준이 맞는지, 더 낮춰도 되는지" 질의. 인스타
      그램(1080px)/카카오톡(780~1200px)/네이버(1000~1600px)/페이스북
      (2048px 고화질) 등 실제 사례를 근거로 고정된 표준은 없고 대략
      1000~2000px 범위임을 답하고, 이 앱의 실제 표시 환경(96px 썸네일 +
      뷰포트 기준 모달, 레티나 배율 포함 실사용 필요 해상도 약 900~1300px)
      을 근거로 1400px을 제안 → 승인받아 `OUTPUT_MAX_LONG_SIDE`를
      1600 → 1400으로 조정.
      (완료: 2026-09-15, 상세: implementation/2026-09-14-write-flow-performance.md
      "2026-09-15 후속" 항목)
- [x] Step 159 (사용자 지시, 2026-09-15): "우리 여기말고도 다른거 이벤트
      픽이라던가 이미지 사용하는것도 1400px보다 더 큰거 들어오면 1400px로
      바꿔서 저장하도록 해... 혹시 현재 1400px 이상인것 저장된것도 있으면
      다 확인하고 바꿔서 다시저장하고.." — 전체 코드베이스 검색 결과,
      사용자/관리자 이미지 업로드 경로는 mom-pick-post-images/
      spot-curation-images 두 버킷뿐이고 "이벤트픽" 업로드 경로는 존재하지
      않음(events는 공공데이터 자동수집 + 외부 URL 참조뿐)을 먼저 확인.
      sharp를 직접 의존성으로 추가(기존엔 Next.js의 선택적 트랜지티브
      의존성일 뿐이었음, 부수적으로 sharp 자체 취약점도 해소)하고, 신규
      resize-for-storage.ts로 두 업로드 라우트 모두 서버 측에서 1400px
      초과 이미지를 강제로 다운스케일하도록 수정(클라이언트 처리를
      우회해도 항상 적용되는 최종 방어선). 기존 저장 이미지도 임시
      스크립트로 두 버킷 전체(23개 파일)를 스캔해 1400px 초과 11개(전부
      mom-pick-post-images)를 같은 경로에 재업로드로 교체 — 최대
      4,000,511B → 244,623B(94% 감소). Storage list() 메타데이터로 갱신
      확인(단, 공개 URL 응답 자체는 cacheControl max-age=3600 때문에 최대
      1시간까지 예전 파일을 보여줄 수 있음 — 저장소 자체는 정상 갱신됨을
      확인했으므로 실제 버그 아님).
      (완료: 2026-09-15, 상세: implementation/2026-09-15-storage-image-downscale.md)
- [x] Step 160 (사용자 지시, 2026-09-15): "지금 내가 맘스픽에서 글 5개를
      올렸어 로그인해서.. 근데.. 그러면 새싹맘에서 한등급 업하는거 아니야?
      아니면 등급 업 기준이 뭐였지?" — spec 기준(월 5회 이상 글쓰기 →
      우수맘, 그 중 채택수 상위 10명 → 파워맘) 확인 후 실제 DB 조회: 계정
      (하린맘, 799c25c1...)이 이번 달 글 5개를 썼는데도 등급이 여전히
      sprout임을 확인. 등급은 매일 새벽 4시 23분(KST) 배치
      (mom-pick-grade-batch.mjs, GitHub Actions 스케줄)가 재계산하는데
      마지막 글 작성 이후 예정된 배치가 실행되지 않았거나 실패한 것으로
      보여(gh CLI 미설치로 Actions 실행 이력 직접 확인은 못함), 배치를
      수동으로 직접 실행해 즉시 반영(sprout → power). 참고로 활동 계정이
      이 하나뿐이라 "상위 10명 정원"에 자동으로 들어가 우수맘을 건너뛰고
      바로 파워맘까지 승급된 것도 함께 확인해 안내함(코드 변경 없음, 순수
      진단+수동 배치 실행).
      (완료: 2026-09-15, 별도 implementation 기록 없음 — 코드 변경이 아닌
      기존 배치 스크립트의 1회성 수동 실행)
- [x] Step 161 (사용자 지시, 2026-09-15): "맞아 그게 맞는듯.. 그렇게
      바꿔줘" — Step 160 진단 중 발견한 파워맘 추천픽 노출 기준 문제(등급만
      보고 글 내용 품질과 무관하게 노출)를 고쳐달라는 승인. getExpertPosts()
      에 `.eq('is_adopted', true)`를 추가해, 상급 등급 작성자의 글이라도
      관리자가 실제로 "채택"한 것만 추천픽에 노출되도록 수정(등급=활동량,
      추천픽 노출=품질 큐레이션으로 분리). 관리자가 이 체크리스트 형식
      글을 어떤 기준으로 채택할지 물어봐, 사진 첨부/자유글 구체성/체크리스트
      완성도 기준의 운영 가이드도 제안(코드 강제 아님, 대화 기록).
      (완료: 2026-09-15, 상세: implementation/2026-09-15-expert-pick-adopted-only.md)

# To-Do List
[개선사항 1] 현재 '맘스픽' 메인 화면의 UI 구조와 접근 제어(Gating) 로직을 아래와 같이 전면 수정해 주세요.

### 📌 [요구사항] 맘스픽 메인 화면 및 권한별 인터랙션 플로우

1. **맘스픽 메인 화면 UI 정돈 (상단 장소 선택 제거)**
   - 맘스픽 메인 화면 진입 시, 상단에 장소 선택 바나 검색창 같은 불필요한 요소가 노출되지 않도록 제거해 주세요.
   - 오직 순수하게 맘스픽 콘텐츠 영역(**파워맘/우수맘 추천, 인기 우수글, 실시간 라이브 글 등**)만 깔끔하고 온전하게 렌더링되도록 뷰를 수정해 주세요.

2. **유저 상태별 접근 제어 (Gating & Interceptor) 로직 적용**
   - 유저가 맘스픽 메인 화면의 콘텐츠를 누르거나 특정 액션을 시도하는 시점에 아래와 같이 분기 처리(인터셉트)를 해주세요.
   - **① 비로그인 사용자:** 
     - 콘텐츠 터치 시, *"로그인이 필요합니다"* 안내 메시지 팝업 노출 후 **[로그인 화면]**으로 이동.
   - **② 로그인 완료 + 첫 글 미작성 사용자:** 
     - 메인 화면 자체는 배경으로 보이되, 화면 전면에 **투명 인터셉트 레이어**를 두거나 터치 시 가로채어 자연스럽게 **[첫 글 쓰기 화면]**으로 유도.
   - **③ 정상 유저 (로그인 + 첫 글 작성 완료):** 
     - 아무런 제약 없이 맘스픽 메인의 모든 콘텐츠와 기능 정상 이용 가능.

[개선사항 2] 글쓰기 화면 장소 선택 에러 버그 수정
현재 유저가 글쓰기 화면에서 장소를 검색할 때('어느 스팟인가요?'), UI에 다음과 같은 치명적인 DB 에러가 빨간색으로 노출되고 있습니다:
- 에러 메시지: `structure of query does not match function result type`

---

### 📌 [요구사항] 장소 검색 쿼리 및 DB 함수 스키마 정합성 맞추기

1. **원인 확인:**
   - 장소 검색 및 반경(Geo-query) / 거리 계산 / 데이터 조회 로직을 수정하는 과정에서, DB에 정의된 커스텀 함수(Function)의 **리턴 타입(Return Type / TABLE 정의)**과 내부의 실제 `SELECT` 쿼리문이 반환하는 컬럼(개수, 순서, 타입) 간의 불일치가 발생했습니다.

2. **조치 사항:**
   - 장소 검색 및 조회를 담당하는 SQL 쿼리문과 PostgreSQL 함수(Function) 정의부를 점검해 주세요.
   - `SELECT` 절에서 반환하는 컬럼(`id`, 상호명, 주소, 좌표, 거리 등)의 구조가 함수 생성 시 선언된 `RETURNS` 타입(또는 매핑 모델)과 **100% 일치**하도록 수정해 주세요.
   - 에러가 유저 화면에 그대로 노출되지 않도록 예외 처리(Try-Catch) 및 로그 처리도 함께 점검해 주세요.

---

[개선사항 3]   현재 이벤트픽 화면 서비스 내에서 제공되는 **모든 '전체보기' 바텀시트의 프리뷰 카드 목록**의 UI 구조와 성능을 일괄적으로 개선해 주세요. 특정 화면뿐만 아니라 아래 대상 섹션의 전체보기 바텀시트에 모두 동일하게 적용해 주세요.

### 📌 [요구사항] 전역 '전체보기' 바텀시트 프리뷰 카드 공통화 (1열 리스트 구조)

1. **적용 대상 (모든 전체보기 바텀시트)**
   - 이벤트픽 (대/중분류 선택 바텀시트)
   - "지금 이순간 함께하기 좋은 알찬 픽" 전체보기 바텀시트
   - "놓치면 후회하는 인기 만점 예약 픽" 전체보기 바텀시트
   - 메인 화면 Hero 카드 영역의 "오늘 전체보기" 바텀시트

2. **공통 프리뷰 카드 UI 스펙 (이미지 제거 및 1열 리스트)**
   - **레이아웃:** 기존의 2열 그리드 구조를 전면 폐기하고, **1열 리스트 형태(한 행에 하나의 카드)**로 공통 컴포넌트화해 주세요.
   - **이미지:** 카드별 이미지 렌더링 로직은 완전히 제거하여 대량 리스트 렌더링 시 버벅임(지연)이 없도록 성능을 최적화해 주세요.
   - **카드 내부 정보 배치 (4단 라인 구조 유지):**
     - **첫 번째 줄:** **제목** (상단 왼쪽, Bold) 및 **현재 위치와의 거리** (상단 오른쪽, 서브 텍스트)
     - **두 번째 줄:** **뱃지 영역** (카테고리 및 상태 뱃지)
     - **세 번째 줄:** **행사 / 운영기간** 
     - **네 번째 줄:** **예약기간**

3. **기대 효과**
   - 앱 전체의 바텀시트 리스트 UI 일관성 확보 및 이미지 로딩 부하 제거를 통한 매끄러운 스크롤/조회 성능 달성.

[개선사항 4] 현재 이벤트픽 '전체보기' 바텀시트 진입 시, 가장 왼쪽 중분류가 자동 포커스되면서 데이터를 불러오는 과정의 UX와 반응 속도를 아래와 같이 개선해 주세요.

---

### 📌 [요구사항] 중분류 자동 포커스 및 데이터 로딩 반응 속도 최적화

1. **중분류 탭 초기 포커스 및 즉각적인 UI 피드백**
   - 바텀시트가 열릴 때 가장 왼쪽 중분류 탭이 자동 선택되는데, 이 과정에서 **탭의 'Active(선택된 상태)' 디자인이 지연 없이 즉시 반영**되도록 탭 렌더링 우선순위를 높여주세요.

2. **데이터 로딩 중 스켈레톤 UI (Skeleton Loader) 적용**
   - 중분류 탭이 선택되거나 변경되어 새로운 데이터를 불러오는 동안, 리스트 영역에 **가벼운 스켈레톤 프리뷰 카드**를 노출하여 유저가 로딩 중임을 명확히 인지할 수 있게 해주세요.
   - 빈 화면이나 멈춘 듯한 느낌이 들지 않도록 시각적 피드백을 반드시 추가해 주세요.

3. **데이터 펫치(Fetch) 및 렌더링 성능 최적화**
   - 초기 진입이나 탭 전환 시 동일한 API가 중복 호출되거나 렌더링이 병목을 일으키는 원인이 있는지 점검하고, 데이터가 들어오는 대로 1열 리스트가 부드럽게 꽂히도록 최적화해 주세요.

---

[개선사항 5] 현재 이벤트픽 서비스 내 모든 '전체보기' 바텀시트 목록의 데이터 정렬 기준을 **'현재 내 위치 기준 거리 가까운 순'**으로 일괄 적용해 주세요. 기존의 섹션별 필터링 조건은 그대로 유지한 상태에서 정렬만 거리 순으로 변경해야 합니다.
추가 사항 - 지역 필터링: 유저의 현재 위치 기반(또는 수동 선택된) 도 단위(예: 서울/경기 등)로 대상 데이터 1차 필터링. (스팟픽과 동일한 구조)
거리 계산 및 정렬: 해당 도(Do) 내의 데이터들만 대상으로 현재 좌표 기준 거리 계산 ➡️ ORDER BY distance ASC 정렬.
페이지네이션: 정렬된 결과물 중 상위 N개씩 끊어서 프론트엔드로 전달 (무한 스크롤 연동).

### 📌 [요구사항] 전체보기 바텀시트 '거리 가까운 순(ASC)' 정렬 적용

1. **적용 대상 및 섹션별 필터링 조건 + 거리 정렬 규칙**
   - **① 메인 히어로 '오늘 전체보기':** 
     - 필터: 오늘 당일 진행되는 데이터만 추출
     - **정렬: 현재 위치 기준 거리 가까운 순 (ASC)**
   - **② "지금 이 순간 함께하기 좋은 알찬 픽" 전체보기:** 
     - 필터: 현재 운영 중인 기간 내에 오늘 날짜가 포함된 항목
     - **정렬: 현재 위치 기준 거리 가까운 순 (ASC)**
   - **③ "놓치면 후회하는 인기 만점 예약 픽" 전체보기:** 
     - 필터: 현재 예약 접수 기간 내에 오늘 날짜가 포함된 항목
     - **정렬: 현재 위치 기준 거리 가까운 순 (ASC)**
     - *(참고: 이벤트픽 대/중분류 선택 바텀시트 역시 동일하게 해당 중분류 기준 + 거리 가까운 순으로 정렬되도록 검토)*

2. **기술적 구현 참고 사항**
   - 유저의 현재 좌표(Latitude, Longitude)를 기반으로 Haversine 공식 또는 PostGIS 등을 활용하여 거리(`distance`)를 계산한 뒤, SQL 쿼리 또는 클라이언트 정렬 시 `ORDER BY distance ASC` 조건이 올바르게 반영되도록 구현해 주세요.
   - 페이지네이션(무한 스크롤) 시에도 거리순 정렬 누락이나 중복 데이터가 발생하지 않도록 쿼리 기준점을 탄탄하게 잡아주세요.
  
[개선사항 6] 이벤트 상세카드를 하기 구조에 맞춰서 재구성해줘

이벤트 상세 카드 8단 구조 스펙
1단: 최상단 이미지 영역

대표 이미지 슬라이드 (여러 장일 경우 좌우 넘겨보기 지원)
예외 처리: 이미지가 아예 없는 경우 해당 영역 통째로 숨김 처리(Gone)

2단: 뱃지 영역

카테고리 뱃지 및 현재 진행 상태 뱃지 노출

3단: 이벤트 명 & 거리 (인터랙티브)

좌측: 이벤트 명 (Bold 체로 강조)

우측: 현재 위치 기준 거리 (🧭 X.Xkm ›)

인터랙션: 우측 거리 영역 탭 시 곧바로 인앱 길찾기 API 실행

4단: 이벤트(운영) 기간

행사가 실제로 진행되고 운영되는 기간 명시

5단: 예약 기간

티켓팅 및 사전 접수가 열려 있는 기간 명시

6단: 상세 설명 (텍스트 토글)

기본 2줄 요약 노출 후 하단 [더보기] 버튼 누르면 전체 내용 확장, [접기] 누르면 다시 축소

7단: 인앱 지도 & 스팟 마커 (미니 맵)

이벤트 개최 장소의 위치를 시각적으로 확인시켜 주는 가벼운 지도 컴포넌트 배치

8단: 하단 고정 CTA 버튼

예약 상품인 경우: [예약하기] (외부 예약 페이지/서울공공예약 링크 연동)

예약이 없는 일반/공공 행사인 경우: [이 장소로 길찾기] 등 핵심 액션 버튼

---

[개선사항 7] 현재 개발 중인 이벤트픽 관련 데이터 파싱 로직과 관리자 화면에 아래의 기능들을 추가 및 고도화해 주세요.

### 📌 [요구사항] 가격 크롤링 Fallback 및 관리자 블로그 큐레이션 연동

1. **가격 정보 파싱 고도화 (URL 크롤링 Fallback)**
   - 이벤트 원천 데이터의 텍스트(Description) 설명에서 가격 정보를 우선적으로 파싱합니다.
   - 만약 무료가 아닌 유료 이벤트로 추정되나 설명에 가격이 없고, **원천 URL(`source_url`)이 존재하는 경우**, 해당 URL에 접근하여 가격 정보를 추출하는 크롤링 Fallback 로직을 구현해 주세요.
   - 없으면 가격정보에 대하여 null로 비워둘것.. 가격에 대하여 15000원 이런식으로 되어있을 수 있고 성인 15000원 어린이 10000원 이런식으로 되어있을 수도 있음.. DB에 데이터는 유연하게 적재할 수 있도록 하는 것이 필요.

2. **관리자 화면 이벤트 상세 팝업 내 '블로그 큐레이션' 기능 추가**
   - 관리자 화면의 `Events` 탭에 있는 개별 이벤트 항목의 **[상세 팝업]** 내부에 **[블로그 큐레이션]** 버튼을 추가해 주세요.
   - 관리자가 해당 버튼을 통해 특정 이벤트와 연관된 양질의 블로그 후기/안내 링크를 검색하고 등록·수정·삭제할 수 있는 관리 UI 및 DB 연동 파이프라인을 구축해 주세요.
  
[개선사항 8] 
유저용 이벤트 상세 화면 내 '큐레이션 블로그' 연동 구조
데이터 파이프라인 (Admin ➡️ DB ➡️ User)

관리자 화면에서 블로그 후보 3개 중 체크하여 저장한 URL들이 해당 이벤트 데이터의 필드(예: curated_blog_urls)에 안전하게 저장됩니다.

유저 상세 화면에서의 노출 (Conditional Rendering)

위치: 6단 상세 설명 아래쪽, 또는 지도 미니맵 위쪽에 [방문 후기 / 추천 블로그] 형태의 깔끔한 카드 리스트로 배치합니다.

조건부 노출: 만약 관리자가 저장한 블로그 URL이 단 1개도 없다면 해당 섹션 전체를 통째로 숨김(Gone) 처리하여 빈 화면이 뜨지 않도록 깔끔하게 방어합니다.

유저 인터랙션

유저가 상세 페이지를 보다가 큐레이션된 블로그 카드를 탭하면, 인앱 브라우저(또는 외부 브라우저)로 부드럽게 연결되어 생생한 현장 후기나 사진을 바로 확인할 수 있습니다.

---

[개선사항 9] 
현재 데이터 파이프라인에서 상시성 스팟(`open_spaces`)과 시한성 이벤트(`events`)의 원천 데이터 수집 및 라우팅 로직을 이분법적으로 명확하게 분리하고, 기존에 섞이거나 크로스되던 모호한 로직을 깔끔하게 정리(Refactoring)해 주세요.

### 📌 핵심 요구사항

1. **엄격한 이분법적 수집 라우터 (Ingestion Router) 구축**
   - 원천 데이터의 속성(운영 기간 유무 등)을 기준으로 수집 단계에서부터 데이터가 완전히 갈라져서 적재되도록 라우팅 로직을 수정해 주세요.
   - **시한성/기간성 데이터 (시작일·종료일 존재):** 무조건 `events` 테이블로만 적재.
   - **상시성/고정 시설 데이터 (기간 없음, 연중 상시):** 무조건 `open_spaces` 테이블로만 적재.
   - 서로 무분별하게 크로스되거나 섞여 들어가는 기존의 모호한 쿼리/로직은 전면 차단하고 제거해 주세요.

데이터 수집 파이프라인에서 '서울시 공공서비스 예약' 등에서 수집되는 공공 대관 및 예약형 시설/프로그램 데이터가 잘못 적재되고 있는 라우팅 경로를 수정해 주세요.

### 📌 수정 요구사항

1. **'서울시 공공예약' 소스 데이터의 라우팅 변경**
   - 수집처가 '서울시 공공서비스 예약'이거나, 핵심 운영 방식이 '대관/사전 예약제'인 공공 체육시설·프로그램 데이터는 `open_spaces`(상시 스팟)가 아닌 **`events` (시한성 이벤트/예약 픽) 테이블로 강제 라우팅**되도록 수정해 주세요.
   - 현재 체육시설 대분류와 축구장, 농구장 등의 시설 대관 예약관련 seoul_public_reservation 원천 데이터 관련 open_spaces에서 보이고 있는데 evnets쪽으로 이관해주세요
2. **기존 잘못 적재된 데이터 마이그레이션/차단**
   - 이미 `open_spaces`에 섞여 들어간 공공 예약/대관 데이터들을 식별하여 `events` 쪽으로 올바르게 재배치(또는 마이그레이션)하고, 앞으로는 원천 소스 및 속성 기준으로 두 영역이 완전히 분리되어 적재되도록 정비해 주세요.

---

[개선사항 10]
시한성 이벤트(`events`)와 상설 스팟(`open_spaces`)을 파이프라인 단계에서는 분리하되, 데이터 모델과 UI 상에서 유기적으로 연결(양방향 링킹)될 수 있도록 구조를 구현해 주세요.

### 📌 핵심 요구사항

1. **데이터 모델링 (FK 링킹)**
   - `events` 테이블에 `open_spaces`를 참조할 수 있는 외래키 컬럼(`open_space_id`)을 추가해 주세요.
   - 이벤트 수집 시 장소 매칭 로직을 두어, 일치하는 상설 스팟이 있으면 자동 매칭하고, 매칭되는 스팟이 없는 경우 관리자가 수동으로 스팟을 지정하거나 신규 등록하여 연결할 수 있도록 관리자 툴 플로우를 지원해 주세요.

2. **유저 화면 양방향 연동**
   - **Event ➔ Spot:** 이벤트 상세 화면의 장소/위치 영역을 탭했을 때, 연결된 `open_spaces` 상세 페이지로 이동할 수 있도록 연동해 주세요.
   - **Spot ➔ Event:** 상설 스팟(`open_spaces`) 상세 페이지 내에, **현재 해당 장소에서 진행 중이거나 예정된 시한성 이벤트(`events`) 목록**을 실시간으로 조회하여 보여주는 섹션을 추가해 주세요. (진행 중인 이벤트가 없는 경우 해당 섹션은 숨김 처리)
