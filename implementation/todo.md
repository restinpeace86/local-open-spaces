...
- [ ] **수집 파이프라인 자동 재시도(Retry) 메커니즘 도입**
  - API 호출 일시적 타임아웃, 네트워크 불안정, DB 부하 등으로 실패 시 단 한 번의 실패로 당일 데이터가 유실되지 않도록 코드 레벨(어댑터/업서트 부) 및 워크플로 레벨의 재시도 전략 구현
  - 실패 시 일정 시간(예: 10~20분 뒤) 간격을 두고 자동 재시도하도록 처리
