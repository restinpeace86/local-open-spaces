...
- [ ] **[긴급] 최근 3일간(8월 26일~28일) 신규 이벤트 수집 0건 원인 진단 및 복구**
  ...
  - 로그 점검: 서울시/경기도/TourAPI 어댑터 및 수집 스크립트 에러 로그 확인
  - 조치: 수집 파이프라인 정상화 및 누락분 수동/자동 백필 실행

- [ ] **어드민 데이터 그리드 배치 수집 현황 모니터링 기능 추가**
  - 상단 요약 카터 및 필터 추가: 오늘 자정 이후 배치로 반영된 데이터 현황(신규 `created_at` vs 갱신 `updated_at`) 표시
  - 그리드 내 시각적 구분: 오늘 수집/갱신된 데이터에 `[NEW]` / `[UPDATED]` 뱃지 또는 하이라이트 적용
