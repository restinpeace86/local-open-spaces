...
## 후속 수정 1 — 다른 카테고리로 뱃지 키가 새는 버그 (Step 55, 같은 날)
사용자 질문("놀이방식당에 기존 수동 체크 뱃지 건드린 거 아니겠지?")을 검증하다
발견: `naver-crawl` API의 `badgeVoteHints`는 스팟의 실제 카테고리와 무관하게
매칭된 리뷰 투표 코드를 전부 내려주므로, 놀이방식당처럼 다른 category_min
스팟도 리뷰에 "주차하기 편해요" 등이 많으면 `jj_parking` 같은 놀이방찜질방/
스파 전용 뱃지 키가 `curation_badges`에 섞여 들어갈 수 있었다. 운영 DB를
직접 조회해 실제로 발생한 사례가 없음을 확인한 뒤(배포 직후라 아직 아무도
트리거 안 함), `isKnownCurationBadgeKey`로 현재 `curationCategoryId`에 실제
존재하는 뱃지 키만 걸러 자동 체크/저장하도록 `spot-curations-panel.tsx`를
고쳤다.

## 후속 수정 2 — 뱃지 세트 정제 (Step 56, 같은 날)
사용자 피드백으로 세 가지 조정:
1. **세신 서비스 우수 삭제** — 뱃지 옵션·키워드·`NAVER_REVIEW_VOTE_CODE_TO_
   BADGE_KEY`의 `scrubber_good` 매핑 전부 제거.
2. **식당(키즈메뉴) → 식당 있음 / 키즈 메뉴 있음 분리** — 사용자 지적: "식당이
   있으면서 식당에 키즈메뉴가 있는것만 체크되는거야?" → 확인해보니 OR 매칭
   이라 "식당은 있는데 키즈메뉴는 없는 곳"과 구분이 안 됐다. `jj_restaurant`
   (구내식당/찜질방 식당/식당)와 `jj_kids_menu`(키즈메뉴/어린이메뉴/미역국/
   돈까스/아기식판)로 분리했다.
3. **야외 족욕탕/휴게공간 → 족욕탕 / 야외 휴게공간 분리** — 사용자 지적: "족욕탕이
   야외에 있지않고 찜질방 내부에 있으면 해당안되는거야?" → `jj_foot_bath`
   (족욕탕)는 "야외" 제한을 빼고 실내외 무관하게 족욕 시설 자체만 판단하도록
   키워드를 좁혔고(`족욕탕`/`족욕장`/`족욕`), `jj_outdoor_lounge`(야외
   휴게공간, `야외 테라스`/`야외 휴게실`/`야외 쉼터`/`야외 벤치`/`야외 정원`)
   를 별도 뱃지로 신설했다.
- 이성 혼탕 나이 제한은 "일단 두자"는 사용자 확인대로 유지.
- 결과: 16개 → 17개 뱃지(식음료 그룹 2→3개, 목욕시설/휴게·청결 그룹 구성은
  동일 개수 유지하며 항목만 교체).
- 검증: `npx tsc --noEmit`/`npm run test`(203개 파일 2,363개)/`npm run build`
  모두 통과.
