...
잘못 체크된(DB에 이미 저장된) 뱃지를 해제해야 하면 기존처럼 블로그 뱃지
큐레이션 화면(전체 토글 가능)에서 처리해야 한다.

## 후속 수정 — 해제 방지 범위 정정 (2026-09-19)
사용자 지적: "내가 여기서 체크 안된거에 대하여 체크했는데.. 그다음에 그거
체크해제하려니깐 안되네.. 저장 안된거 여기서 누른거 관련해서는 다시 해제
해야하는거 아니야 ? db에서 불러온거 말고?"

첫 구현이 잘못됐다 — "해제 불가"를 `selectedBadges`에 체크된 모든 것(=이미
저장된 것 + 이번 화면에서 방금 추가한 것 전부)에 걸어버려서, 관리자가 이
화면에서 방금 수동으로 켰거나 크롤링이 자동으로 매칭해 켠 것(둘 다 아직 저장
전, 초록색)까지 다시 끌 수 없었다. 원래 요구사항은 "DB에 이미 저장돼 있던
뱃지"(`savedBadgeKeys`, 파란색)만 해제 금지 대상이었다.

`handleToggleBadge(key)`를 `savedBadgeKeys.has(key)`일 때만 조기 반환(해제
금지)하도록 수정하고, 그 외에는 정상적으로 체크/해제를 토글한다. 체크박스의
`disabled` 조건도 `checked`(방금 체크한 것 포함 전부) → `isSaved`(DB 저장분만)
로 정정. 초록색(저장 전) 뱃지는 이제 자동 매칭됐든 수동으로 켰든 클릭 한 번에
다시 꺼진다 — 자동 매칭이 잘못 켰을 경우에도 저장 전이라면 똑같이 바로잡을 수
있어 일관적이다.

**검증**: 신규 테스트 2개 추가 — "DB에 저장되지 않은 뱃지(방금 수동 체크)는
다시 눌러서 해제 가능", "DB에 저장되지 않은 뱃지(크롤링 자동 매칭)도 다시
눌러서 해제 가능"(둘 다 `disabled=false` 확인 + 클릭 후 `checked=false` 확인).
기존 "이미 저장된 뱃지는 해제 안 됨" 테스트는 그대로 통과(회귀 없음). 전체
166개 파일/1975개 테스트, `npx tsc --noEmit`/`npm run build` 모두 통과.
