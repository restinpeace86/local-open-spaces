...
- **실제 브라우저 검증에서 발견/수정한 배포 후 버그**: 실제 파트너 계정
  ...
  드러났다.
- 위 수정 배포 후 재검증 완료: 상품 관리에서 팀당(50,000원)/인당(15,000원)
  상품 등록 → 목록에 "50,000원 · 팀당(인원수 무관)"/"15,000원 ·
  인당(인원수만큼 곱함)" 정상 표시 확인 → 예약 추가 화면 콤보박스에 두
  상품이 가격과 함께 노출 확인 → 인당 상품 선택 시 1명 기준 15,000원 자동
  채움, 인원을 3명으로 늘리면 45,000원으로 재계산 확인 → 팀당 상품으로
  바꾸면 인원수와 무관하게 50,000원 유지 확인 → 테스트로 만든 상품 전부
  정리(실사용 계정 데이터 오염 방지) 완료, `partner_products` 최종 상태
  빈 배열 확인.
