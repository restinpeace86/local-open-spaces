...
## 추가 정밀화 (2026-08-21, 사용자 지시)
- `is_kids_friendly`를 시설명/사업장명 키워드 매칭으로 정밀화: 지정 키워드(어린이/유아/키즈/영유아/유아풀/어린이풀/키즈풀)가 API1 `faci_nm` 또는 API2 `BPLC_NM`에 포함되면 `true`, 아니면 기존대로 `false`.
- 사용자 지시문의 "시설명/사업장명/상세설명 필드" 중 "상세설명"은 두 API 어디에도 실제로 존재하지 않음을 실측으로 재확인(API1: `faci_nm` 외 텍스트 필드 없음, API2: `BPLC_NM` 외 텍스트 필드 없음) — 있지도 않은 필드를 매칭 대상으로 넣지 않고 실제 존재하는 이름 필드에만 적용했다(추측 금지).
- 단위 테스트 12건 추가(키워드별 파라미터화 테스트 7건 + 사업장명 매칭 1건 + 미매칭 시 false 유지 1건 + `matchesKidsKeyword` 헬퍼 단독 테스트 3건) — 총 26/26 통과.
- 실제 데이터 재검증: 1,537건 중 215건이 키워드 매칭됨(예: "덕업관어린이수영장", "주식회사 키즈스윔 오송지점", "오션키즈 성북점") — 실제 upsert로 반영 완료.
